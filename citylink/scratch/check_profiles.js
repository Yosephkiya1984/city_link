...
async function test() {
  for (const n of numbers) {
    console.log(`Checking profile for ${n}...`);
    const { data, error } = await client.from('profiles').select('*').eq('phone', n).maybeSingle();
    if (error) {
      console.log(`❌ ${n}: ${error.message}`);
    } else if (data) {
      console.log(`✅ ${n}: Found (ID: ${data.id}, Role: ${data.role})`);
    } else {
      console.log(`❓ ${n}: Not found in profiles`);
    }
  }
}

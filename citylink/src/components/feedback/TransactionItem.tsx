...
export function TransactionItem({ tx, index }) {
  ...
  const handlePressIn = () => {
    Animated.spring(scaleAnim, { toValue: 0.96, useNativeDriver: true, tension: 200, friction: 4 }).start();
    ...
    <Animated.View style={{
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: C.text, fontSize: FontSize.md, fontFamily: Fonts.bold, marginBottom: 2 }}>
          ...
          <View style={{ alignItems: 'flex-end' }}>
            <Text style={{ color: iconColor, fontSize: FontSize.lg, fontFamily: Fonts.black, marginBottom: 2 }}>
              {isCredit ? '+' : '-'}{fmtETB(tx.amount, 0)}
            </Text>
            <View style={{
              paddingHorizontal: 6,
              paddingVertical: 2,
              ...
              <Text style={{ color: iconColor, fontSize: FontSize.xs, fontFamily: Fonts.bold, textTransform: 'uppercase' }}>
                {tx.category}
              </Text>
            </View>
          </View>

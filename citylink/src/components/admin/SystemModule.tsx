    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: theme.sub }]}>
          Manage ecosystem nodes and global configurations
        </Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.nodeGrid}>
          <NodeCard
            ...
            status={nodes.auth}
            latency="1.2ms"
            color={nodes.auth === 'ACTIVE' ? theme.green : theme.red}
          />
          <NodeCard
            name="Supabase DB"
            status={nodes.database}
            latency="4ms"
            color={nodes.database === 'HEALTHY' ? theme.green : theme.red}
          />
          <NodeCard
            name="Object Storage"
            status={nodes.storage}
            latency="88ms"
            color={theme.green}
          />
    ...
    <View style={styles.configItem}>
      <View style={{ flex: 1 }}>
        <Text
          style={[
            styles.configLabel,
            { color: isDanger ? theme.red : theme.text, fontFamily: Fonts.label },
          ]}
        >
          {label}
        </Text>
...
function TariffCard({ label, value, icon }: { label: string; value: string; icon: any }) {
  ...
  return (
    <TouchableOpacity
      onPress={handleEdit}
      style={[styles.tariffCard, { backgroundColor: theme.surface, borderColor: theme.rim }]}
    >
      ...
      <View style={{ marginLeft: 12 }}>
        ...
        <Text style={[styles.tariffValue, { color: theme.text, fontFamily: Fonts.label }]}>
          {value}
        </Text>
      </View>

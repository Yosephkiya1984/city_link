...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, Alert, Dimensions, Platform } from 'react-native';
  ...
  const handleReject = (agent) => {
    ...
    if (Platform.OS === 'web') {
      try {
        ...
        if (reason !== null) {
          rejectAgent(agent.id, reason || 'Incomplete documentation')
            .then(res => {
              if (res.error) window.alert(res.error);
              else loadAgents();
            });
  ...
  const handleSuspend = (agent) => {
    ...
    if (Platform.OS === 'web') {
      try {
        if (window.confirm(`${action} ${name}?`)) {
          supaQuery(c => c.from('delivery_agents').update({ agent_status: newStatus }).eq('id', agent.id))
            .then(res => {
              if (res.error) window.alert(res.error);
              else loadAgents();
            });
    ...
    Alert.alert(`${action} Agent`, `${action} ${name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          const res = await supaQuery(c => c.from('delivery_agents').update({ agent_status: newStatus }).eq('id', agent.id));
    ...
    <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.rim }]}>
      <View style={styles.cardHeader}>
        <View style={styles.agentInfo}>
          <View style={[styles.avatar, { backgroundColor: statusColor(item.agent_status) + '20' }]}>
            <Ionicons name={vehicleIcon(item.vehicle_type)} size={20} color={statusColor(item.agent_status)} />
          </View>
        ...
        <View style={[styles.statusPill, { backgroundColor: statusColor(item.agent_status) + '18' }]}>
      ...
      <View style={styles.detailsRow}>
        <View style={styles.detailItem}>
          ...
          <Text style={[styles.detailValue, { color: theme.sub }]}>{item.plate_number || 'N/A'}</Text>
        </View>
        <View style={styles.detailItem}>
        ...
        <View style={styles.detailItem}>
          ...
          <Text style={[styles.detailValue, { color: theme.sub }]}>{item.total_deliveries || 0}</Text>
        </View>
        <View style={styles.detailItem}>
          <Text style={[styles.detailLabel, { color: theme.hint }]}>RATING</Text>
          <Text style={[styles.detailValue, { color: theme.sub }]}>⭐ {Number(item.rating || 5).toFixed(1)}</Text>
        </View>
      </View>
      ...
      <View style={styles.actions}>
        {item.agent_status === 'PENDING' ? (
          <>
            ...
            >
              <Text style={{ color: theme.red || '#fc8181', fontFamily: Fonts.label, fontSize: 12 }}>Reject</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => handleApprove(item)}
              style={[styles.approveBtn, { backgroundColor: theme.primary }]}
            >
              <Text style={{ color: theme.ink, fontFamily: Fonts.label, fontSize: 12 }}>Approve</Text>
            </TouchableOpacity>
          </>
        ) : (
          <TouchableOpacity
            ...
            style={[
              styles.toggleBtn,
              { 
                backgroundColor: item.agent_status === 'SUSPENDED' 
                  ? (theme.green || '#68d391') + '15' 
                  : (theme.red || '#fc8181') + '15',
                borderColor: item.agent_status === 'SUSPENDED'
          ...
          >
            <Ionicons
              ...
              size={14}
              color={item.agent_status === 'SUSPENDED' ? (theme.green || '#68d391') : (theme.red || '#fc8181')}
              style={{ marginRight: 6 }}
            />
            <Text style={{
              color: item.agent_status === 'SUSPENDED' ? (theme.green || '#68d391') : (theme.red || '#fc8181'),
...
const styles = StyleSheet.create({
  ...
  rejectBtn: { flex: 1, borderRadius: Radius.lg, borderWidth: 1, height: 40, alignItems: 'center', justifyContent: 'center' },
  approveBtn: { flex: 2, borderRadius: Radius.lg, height: 40, alignItems: 'center', justifyContent: 'center' },
  toggleBtn: { flex: 1, borderRadius: Radius.lg, borderWidth: 1, height: 40, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },

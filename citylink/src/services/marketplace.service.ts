...
// ── Configuration ─────────────────────────────────────────────────────────────
const PRODUCT_COLS = 'id, name, title, description, price, category, image_url, images_json, stock, status, condition, merchant_id, created_at, updated_at';
const ORDER_COLS = 'id, escrow_id, buyer_id, product_id, quantity, shipping_address, tracking_number, created_at, merchant_id, product_name, qty, total, status, escrow_release_at, updated_at, delivery_pin, expires_at';
...
const attachMerchantProfiles = async (products) => {
  ...
  if (profiles) {
    ...
    return products.map(p => ({
      ...
      merchant_name: profileMap[p.merchant_id]?.merchant_name || profileMap[p.merchant_id]?.full_name
    }));
  }
  return products;
};
...
export async function fetchProducts(limit = 50) {
  const res = await supaQuery((c) =>
    c.from('products').select(PRODUCT_COLS).eq('status', 'active').gt('stock', 0).order('created_at', { ascending: false }).limit(limit)
  );
...
export async function searchProducts(query, limit = 40) {
  ...
  const res = await supaQuery((c) =>
    c.from('products').select(PRODUCT_COLS).eq('status', 'active').gt('stock', 0)
...
export async function fetchProductsByCategory(category, limit = 40) {
  ...
  const res = await supaQuery((c) =>
    c.from('products').select(PRODUCT_COLS).eq('status', 'active').gt('stock', 0)
      .ilike('category', category).order('created_at', { ascending: false }).limit(limit)
  );
  return { ...res, data: await attachMerchantProfiles(res.data) };
}

export async function fetchProductById(productId) {
  return supaQuery((c) => c.from('products').select(PRODUCT_COLS).eq('id', productId).maybeSingle());
}

export async function fetchMerchantInventory(merchantId) {
  return supaQuery((c) =>
    c.from('products').select('*').eq('merchant_id', merchantId).neq('status', 'removed').order('created_at', { ascending: false })
  );
}
...
export async function updateProduct(productId, updates) {
  return supaQuery((c) =>
    c.from('products').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', productId).select().single()
  );
}

export async function deleteProduct(productId) {
  return supaQuery((c) => c.from('products').update({ status: 'removed', updated_at: new Date().toISOString() }).eq('id', productId));
}
...
export async function uploadProductImage(imageData) {
  ...
  try {
    ...
    const { error: uploadError } = await client.storage.from('products').upload(fileName, decode(imageData.base64), { contentType, cacheControl: '3600' });
...
export async function fetchMarketplaceOrdersByBuyer(buyerId) {
  ...
  const res = await supaQuery((c) =>
    c.rpc('fetch_buyer_orders', { p_buyer_id: buyerId })
  );
...
export async function releaseMarketplaceEscrow(orderId, escrowId) {
  ...
  return supaQuery((c) => c.rpc('release_escrow', { 
    p_order_id: orderId, 
...
export async function openMarketplaceDispute(orderId, buyerId, reason) {
  ...
  if (!error && data) {
    await supaQuery(c => c.from('disputes').insert({
      order_id: orderId,
      buyer_id: buyerId,
...
export async function merchantMarkShippedMarketplace(orderId, merchantId, pin) {
  return supaQuery((c) => c.from('marketplace_orders').update({ status: 'SHIPPED', delivery_pin: pin, updated_at: new Date().toISOString() }).eq('id', orderId).eq('merchant_id', merchantId));
}
...
export async function rpcCancelAndRefundOrder(orderId, reason) {
  return supaQuery((c) => c.rpc('cancel_and_refund_marketplace_order', { 
    p_order_id: orderId, 
    p_reason: reason 
  }));
}
...
export async function fetchMerchantMetrics(merchantId) {
  if (!hasSupabase() || !merchantId) return { totalRevenue: 0, activeOrders: 0, totalProducts: 0, lowStock: 0 };
  
  const [{ data: orders }, { data: products }] = await Promise.all([
    supaQuery(c => c.rpc('fetch_merchant_orders', { p_merchant_id: merchantId })),
    supaQuery(c => c.from('products').select('id, stock, status').eq('merchant_id', merchantId).neq('status', 'removed')),
  ]);

  const allActiveStatuses = ['PAID', 'DISPATCHING', 'AGENT_ASSIGNED', 'SHIPPED', 'IN_TRANSIT', 'AWAITING_PIN'];
  const revenueStatuses = [...allActiveStatuses, 'COMPLETED'];

  const totalRevenue = (orders || []).filter(o => revenueStatuses.includes(o.status)).reduce((sum, o) => sum + Number(o.total || 0), 0);
...
export async function fetchMerchantSalesHistory(merchantId) {
  ...
  const { data: allOrders } = await supaQuery(c => c.rpc('fetch_merchant_orders', { p_merchant_id: merchantId }));
  const salesStatuses = ['PAID', 'DISPATCHING', 'AGENT_ASSIGNED', 'SHIPPED', 'IN_TRANSIT', 'AWAITING_PIN', 'COMPLETED'];
  const orders = (allOrders || []).filter(o => salesStatuses.includes(o.status) && o.created_at >= sevenDaysAgo.toISOString());
  ...
  return { curve: values.map((v: number) => v / maxVal), raw: values, labels: Object.keys(dailyMap) };
}
...
export const marketplaceService = {
  ...
  fetchWalletTransactions: async (walletId) => {
    return supaQuery((c) => 
      c.from('transactions').select('*').eq('wallet_id', walletId).order('created_at', { ascending: false }).limit(20)
    );
  },
  ...
  cancelOrder: async (orderId, reason) => {
    ...
    const { data, error } = await supaQuery((c) =>
      c.rpc('cancel_and_refund_marketplace_order', { p_order_id: orderId, p_reason: reason || 'cancelled_by_merchant' })
    );

...
export const updateTicketStatus = async (ticketId: string, status: string) => {
  ...
  return supaQuery((client) => client.from('bus_tickets').update({ status }).eq('id', ticketId));
};
...
export const createRoute = async (route: any) => {
  ...
  return supaQuery((client) => client.from('bus_routes').insert(route));
};
...
export async function buyTransportPass(userId, passType, price, qrToken) {
  return supaQuery((c) =>
    c.rpc('process_transport_pass_atomic', {
      p_user_id: userId,

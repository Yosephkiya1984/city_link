...
const storageKey = (userId: string) => `cl_wallet_pin_v1_${userId}`;

/**
 * SHA-256 hash of the PIN (hex). Never store the raw PIN.
 */
export async function hashWalletPin(plain: string) {
...
export function isValidPinFormat(plain: string) {
...
export async function verifyWalletPin(userId: string, plain: string) {
...
export async function setWalletPin(userId: string, plain: string) {
...
export async function changeWalletPin(userId: string, currentPlain: string, newPlain: string) {

...
export async function fetchCityStats() {
  ...
  try {
    // 1. Total Users Count
    const { data: users } = await supaQuery(c => c.from('profiles').select('id', { count: 'exact', head: true }));
    stats.totalUsers = users ? (users.length > 1000 ? (users.length / 1000).toFixed(1) + 'K' : users.length.toString()) : '1.2k';
    ...
    const { data: rail } = await supaQuery(c => c.from('rail_sessions').select('id', { count: 'exact', head: true }).is('tap_out_time', null));
    stats.lrtRiders = rail ? rail.length.toString() : '45';

    // 4. System Load (Simulated based on hour)
    const hour = new Date().getHours();
    const load = (hour > 8 && hour < 20) ? 15 + Math.floor(Math.random() * 20) : 5 + Math.floor(Math.random() * 5);

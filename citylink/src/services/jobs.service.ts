...
export async function fetchJobs() {
  return supaQuery((c) =>
    c
      .from('job_listings')
      .select('*,profiles(merchant_name,full_name)')
      .eq('status', 'OPEN')
...
export async function fetchMyApplications(userId) {
  return supaQuery((c) =>
    c
      ...
      .select('*,job_listings(title,merchant_id)')
      ...
      data: [
        {
          ...
          salary_range: '20,000-35,000 ETB',
          type: 'FULL_TIME',
        ...
        },
        {
          ...
          salary_range: '15,000-25,000 ETB',
          type: 'FULL_TIME',
      ...
      data: [
        {
          ...
          cover_note: 'Experienced developer with 5+ years in React Native',
        },
        {
          id: 'app2',
          employer_id: employerId,
          ...
          cover_note: 'Recent graduate with strong portfolio',
        },
      ],
      error: null,
    };
...
export const createJobListing = async (job) => {
  ...
  return supaQuery((client) => client.from('job_listings').insert(job));
};

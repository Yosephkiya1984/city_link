...
export function ErrorReportingProvider({ children }: ErrorReportingProviderProps) {
    ...
    try {
      ...
      if (storedErrors) {
        ...
        const stats = errors.reduce((acc, error) => ({
          ...
          criticalErrors: acc.criticalErrors + (error.severity === SEVERITY_LEVELS.CRITICAL ? 1 : 0),
  ...
  const reportErrorsToBackend = async () => {
...
export const ErrorUtils = {
  ...
  createError: (type: string, message: string, severity = SEVERITY_LEVELS.MEDIUM, context: any = null) => ({
    type,
    message,
    severity,
    context,
  ...
  handlePerformanceError: (reportError: Function, metric: string, threshold: number, actual: number) => {
    const errorData = {

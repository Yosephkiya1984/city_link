...
import { fetchSalonBookings, fetchSalonServices, updateBookingStatus, createService } from '../../services/services.service';
...
export default function SalonDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (bookingsRes.data) {
        const sortedBookings = bookingsRes.data.sort((a, b) => new Date(a.appointment_time).getTime() - new Date(b.appointment_time).getTime());
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <View>
                ...
                <Text style={{ color: SALON_COLORS.primary, fontSize: 32, fontFamily: Fonts.black, marginTop: 4 }}>
                  {todayBookings}
                </Text>
              </View>
              <View style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: SALON_COLORS.primaryL, alignItems: 'center', justifyContent: 'center' }}>
                <Ionicons name="calendar" size={24} color={SALON_COLORS.primary} />
              </View>
            </View>
            
            <View style={{ height: 1, backgroundColor: 'rgba(236,72,153,0.2)', marginVertical: 20 }} />
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
              ...
              <View style={{ alignItems: 'center' }}>
                ...
                <Text style={{ color: 'rgba(236,72,153,0.7)', fontSize: 10, fontFamily: Fonts.bold }}>
                  REVENUE
                </Text>
              </View>
            </View>
        ...
        {activeTab === 'bookings' && (
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {loading && bookings.length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>Loading bookings...</Text>
            ) : bookings.length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>No bookings yet</Text>
            ) : (
        ...
        {activeTab === 'services' && (
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {loading && services.length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>Loading services...</Text>
            ) : services.length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>No services yet</Text>
            ) : (
                ...
                <Card key={service.id} style={{ marginBottom: 8, padding: 12 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: SALON_COLORS.primary, fontSize: 12, fontFamily: Fonts.bold }}>
                ...
                {[15, 30, 45, 60, 90, 120].map(duration => (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      paddingVertical: 8,
                      borderRadius: 8,
                      backgroundColor: newService.duration_minutes === duration ? SALON_COLORS.primaryL : C.surface,
                      borderWidth: 1,
                      borderColor: newService.duration_minutes === duration ? SALON_COLORS.primaryB : C.edge2,
                      alignItems: 'center'
                    }}
                  >
                    <Text style={{ 
                      color: newService.duration_minutes === duration ? SALON_COLORS.primary : C.sub, 
                      fontSize: 10, 
                      fontFamily: Fonts.bold 
                    }}>
                      {duration}m
          ...
          <View style={{ flex: 1, backgroundColor: C.ink }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              <Card style={{ padding: 16, marginBottom: 16 }}>
                ...
                <View style={{ marginBottom: 12 }}>
                  <Text style={{ color: SALON_COLORS.primary, fontSize: 18, fontFamily: Fonts.black }}>

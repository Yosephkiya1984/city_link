...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, Image, TextInput, Dimensions, Alert, RefreshControl } from 'react-native';
...
import { findNearbyAgents, dispatchOrderToAgents, getCurrentLocation } from '../../services/delivery.service';
...
export default function ShopDashboard() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', price: '', category: 'Electronics', stock: '10', description: '', condition: 'new' });
  const [selectedImage, setSelectedImage] = useState(null);
  const [uploading, setUploading] = useState(false);
  const [showShipSuccess, setShowShipSuccess] = useState(null);
  const [walletTransactions, setWalletTransactions] = useState([]);
  const [withdrawing, setWithdrawing] = useState(false);
  const [salesHistory, setSalesHistory] = useState({ curve: [0,0,0,0,0,0,0], raw: [], labels: [] });
  ...
  const loadData = React.useCallback(async () => {
    ...
    // Get wallet ID first
    const { data: wallet } = await supabase.from('wallets').select('id, balance').eq('user_id', currentUser.id).single();
  ...
  React.useEffect(() => {
    ...
    const chOrd = subscribeToTable(`merchant-orders-${Date.now()}`, 'marketplace_orders', `merchant_id=eq.${currentUser.id}`, (payload) => {
    ...
    const chInv = subscribeToTable(`merchant-inventory-${Date.now()}`, 'products', `merchant_id=eq.${currentUser.id}`, (payload) => {
      if (payload.eventType === 'UPDATE') {
        setInventory(prev => prev.map(p => p.id === payload.new.id ? { ...p, ...payload.new } : p));
      } else if (payload.eventType === 'INSERT') {
  ...
  const handleSaveProduct = async () => {
    ...
    let finalImageUrl = editingProduct?.image_url || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=400&auto=format&fit=crop';
    ...
    setNewProduct({ name: '', price: '', category: 'Electronics', stock: '10', description: '', condition: 'new' });
  ...
  const handleDeleteProduct = async (productId) => {
    Alert.alert(
      'Delete Product',
      'Remove this product from your store? This cannot be undone.',
      [
  ...
  const handleConfirmPickup = async (orderId) => {
    ...
    setLoading(true);
    try {
      const { success, status, delivery_pin } = await marketplaceService.confirmPickup(orderId, currentUser.id);
      if (success) {
        if (status === 'SHIPPED') {
  ...
  const handleSwitchSelfDelivery = async (order) => {
    Alert.alert(
      "Self-Delivery",
      "Cancel the agent search and deliver this yourself?",
      [
        { text: "No", style: "cancel" },
        { 
          ...
          onPress: async () => {
            ...
            try {
              ...
              await supabase.from('delivery_dispatches').update({ status: 'SUPERSEDED' }).eq('order_id', order.id);
              await supabase.from('marketplace_orders').update({ status: 'SHIPPED', dispatch_expires_at: null }).eq('id', order.id);
  ...
  const submitSelfDeliveryPin = async () => {
    ...
    const { ok, error } = await confirmDeliveryWithPin(pinPromptOrder.id, pinInput.trim(), null, currentUser.id);
  ...
  const handleMessageBuyer = async (order) => {
    ...
    try {
      ...
      const { data: existing } = await supabase.from('message_threads').select('thread_id').eq('thread_id', threadId).maybeSingle();
        ...
        { 
          ...
          onPress: async () => {
            ...
            try {
              ...
              const { data: wallet } = await supabase.from('wallets').select('id').eq('user_id', currentUser.id).single();
              if (wallet) {
                const txId = uid();
                ...
                const { error: balErr } = await supabase.from('wallets').update({ balance: 0 }).eq('id', wallet.id);
  ...
  const renderOverview = () => {
    ...
    const activeOrd = orders.filter(o => ['PAID', 'SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status)).length;
    ...
    return (
      <View style={styles.tabContent}>
        <View style={styles.headerTitleRow}>
          ...
          <TouchableOpacity style={styles.iconButtonOutlined} onPress={() => showToast('Report downloaded to device', 'success')}>
            ...
            {inventory.slice(0, 3).map((item, i) => (
              <View key={i} style={styles.topSellingItem}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.tsSales}>{item.stock} in stock â€¢ {item.category}</Text>
                </View>
                <Text style={styles.tsPrice}>ETB {item.price.toLocaleString()}</Text>
              </View>
            ))}
            {inventory.length === 0 && <Text style={{ color: T.textSub, textAlign: 'center' }}>No products listed</Text>}
          </View>
        </View>
      </View>
    );
  ...
  const renderInventory = () => (
    <View style={styles.tabContent}>
      <View style={styles.headerTitleRow}>
        ...
        <TouchableOpacity style={styles.primaryButtonSolid} onPress={() => setShowProductModal(true)}>
      ...
      <View style={styles.filterRowMobile}>
        ...
        {inventorySearch.length > 0 && (
          <TouchableOpacity style={styles.iconButtonOutlined} onPress={() => setInventorySearch('')}>
      ...
      <View style={{ gap: 12 }}>
        {inventory.filter(p => !inventorySearch || p.name?.toLowerCase().includes(inventorySearch.toLowerCase())).length === 0 && !loading && (
          <View style={{ alignItems: 'center', padding: 40 }}>
            <Ionicons name="cube-outline" size={48} color={T.border} />
            <Text style={{ color: T.textSub, marginTop: 12 }}>{inventorySearch ? 'No matching products' : 'No products in stock'}</Text>
          </View>
        )}
        {inventory.filter(p => !inventorySearch || p.name?.toLowerCase().includes(inventorySearch.toLowerCase())).map(p => {
            ...
            <View key={p.id} style={styles.productMobileCard}>
              <View style={{ flexDirection: 'row', gap: 12 }}>
                ...
                <View style={{ flex: 1, justifyContent: 'center' }}>
                  <Text style={styles.tsName} numberOfLines={1}>{p.name}</Text>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 16 }}>
                ...
                <TouchableOpacity style={styles.deleteBtnSmall} onPress={() => handleEditProduct(p)}>
                  <Ionicons name="create-outline" size={14} color={T.primary} />
                </TouchableOpacity>
                <TouchableOpacity style={[styles.deleteBtnSmall, { marginLeft: 8 }]} onPress={() => handleDeleteProduct(p.id)}>
                  <Ionicons name="trash-outline" size={14} color={T.tertiary} />
                </TouchableOpacity>
                <View style={{ flex: 1 }} />
                <Text style={styles.stockText}><Text style={{ color: p.stock > 0 ? T.text : T.tertiary }}>{p.stock || 0}</Text>/{maxStock}</Text>
                <View style={styles.stockBarBgMob}>
                  <View style={[styles.stockBarFill, { width: `${((p.stock || 0) / maxStock) * 100}%`, backgroundColor: barColor }]} />
                </View>
              </View>
            </View>
          );
    ...
    <View style={styles.tabContent}>
      <View style={styles.headerTitleRow}>
        <View>
          ...
          <Text style={styles.pageSubtitle}>{orders.filter(o => ['PAID', 'SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status)).length} active shipments</Text>
        </View>
      </View>
      ...
      {openDisputes.length > 0 && (
        <View style={styles.alertBanner}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.alertTitle}>{openDisputes.length} Open Dispute{openDisputes.length > 1 ? 's' : ''}</Text>
            <Text style={styles.alertSub}>{openDisputes[0]?.reason || 'Needs resolution within 24h.'}</Text>
          </View>
        </View>
      )}
          ...
          <View key={o.id} style={styles.orderMobileCard}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={styles.ocTimeTxtMobile}> â€¢ {new Date(o.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
              </View>
              <View style={[styles.ocStatusBadge, {
                backgroundColor: o.status === 'PAID' ? T.primaryDark + '30' :
                  ['SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status) ? T.secondary + '30' :
                    o.status === 'COMPLETED' ? T.primary + '30' :
                      o.status === 'DISPUTED' ? T.tertiary + '30' : T.surfaceHigh
              }]}>
                <Text style={[styles.ocStatusTxt, {
                  color: o.status === 'PAID' ? T.primary :
                    ['SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status) ? T.secondary :
                      o.status === 'COMPLETED' ? T.primary :
                        o.status === 'DISPUTED' ? T.tertiary : T.textSub
                }]}>{o.status.replace('_', ' ')}</Text>
              </View>
            </View>

            <View style={{ flexDirection: 'row', gap: 12, marginTop: 16 }}>
              <View style={[styles.ocImgMobile, { alignItems: 'center', justifyContent: 'center' }]}>
                <Ionicons name="cube" size={24} color={T.primary} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.ocProdName}>{o.product_name}</Text>
                <Text style={styles.ocProdDetail} numberOfLines={1}>Quantity: {o.quantity || o.qty || 1}</Text>
                <Text style={styles.ocBigAmountMobile}>ETB {Number(o.total)?.toLocaleString()}</Text>
              </View>
            </View>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <View style={{ flex: 1, paddingLeft: 12, borderLeftWidth: 1, borderLeftColor: T.border }}>
                <Text style={styles.ocTinyLabel}>{['SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status) ? 'DELIVERY PIN' : 'ESCROW STATUS'}</Text>
                {['SHIPPED', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(o.status) ? (
                  <View style={styles.ocPinBoxMobile}>
                    <Text style={styles.ocPinTxt}>{o.delivery_pin}</Text>
                  </View>
                ) : (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                    <Ionicons name={o.status === 'COMPLETED' ? 'checkmark-circle' : 'lock-closed'} size={14} color={o.status === 'COMPLETED' ? T.primary : T.secondary} />
                    <Text style={[styles.ocLockTxt, o.status === 'COMPLETED' && { color: T.primary }]}>
            ...
            <View style={{ marginTop: 16, flexDirection: 'row', gap: 12 }}>
              {o.status === 'PAID' ? (
                ...
                <View style={{ flexDirection: 'row', gap: 8, flex: 2 }}>
                  <TouchableOpacity style={[styles.ocBtnMobile, { backgroundColor: T.surfaceHigh, flex: 1.2 }]} disabled>
                  ...
                  <TouchableOpacity
                    style={[styles.ocBtnMobile, { backgroundColor: T.surfaceHigh, flex: 1, borderColor: T.border, borderWidth: 1 }]}
                ...
                <View style={{ flex: 2, gap: 8 }}>
                  <TouchableOpacity
                    ...
                    onPress={() => { setPinInput(''); setPinPromptOrder(o); }}
                  >
                    <Ionicons name="checkmark-done" size={14} color="#000" style={{ marginRight: 4 }} />
                    <Text style={[styles.ocBtnTxt, { color: '#000' }]}>Complete Delivery (PIN)</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.ocBtnMobile, { backgroundColor: T.surfaceHigh, borderColor: T.primary + '30', borderWidth: 1 }]}
                    onPress={() => handleDispatchRetry(o.id)}
                  >
                    <Ionicons name="bicycle-outline" size={14} color={T.primary} style={{ marginRight: 4 }} />
                    <Text style={[styles.ocBtnTxt, { color: T.primary, fontSize: 11 }]}>Search for Drivers</Text>
                  </TouchableOpacity>
                </View>
              ) : o.status === 'AGENT_ASSIGNED' ? (
                <View style={{ flex: 2, gap: 8 }}>
                  <TouchableOpacity
                    style={[styles.ocBtnMobile, { backgroundColor: o.merchant_confirmed_pickup ? T.surfaceHigh : T.primary }]}
                    onPress={() => handleConfirmPickup(o.id)}
                    disabled={o.merchant_confirmed_pickup}
                  >
                    <Ionicons name={o.merchant_confirmed_pickup ? 'time-outline' : 'hand-right-outline'} size={16} color={o.merchant_confirmed_pickup ? T.textSub : T.bg} style={{ marginRight: 6 }} />
                    <Text style={[styles.ocBtnTxt, { color: o.merchant_confirmed_pickup ? T.textSub : T.bg }]}>
                      {o.merchant_confirmed_pickup ? 'Awaiting Agent...' : 'Confirm Handover'}
                    </Text>
                  </TouchableOpacity>
                  {o.agent_confirmed_pickup && !o.merchant_confirmed_pickup && (
                     <Text style={{ fontSize: 10, color: T.primary, textAlign: 'center', fontWeight: '800' }}>AGENT HAS CONFIRMED PICKUP</Text>
                  )}
                </View>
              ) : ['IN_TRANSIT', 'AWAITING_PIN'].includes(o.status) ? (
                <TouchableOpacity style={[styles.ocBtnMobile, { backgroundColor: T.surfaceHigh, flex: 2 }]} disabled>
                  <Text style={[styles.ocBtnTxt, { color: T.textSub }]}>
                    {o.status === 'AWAITING_PIN' ? 'Driver Arrived (Awaiting PIN)' : 'Driver on Route to Buyer'}
                  </Text>
                </TouchableOpacity>
              ) : o.status === 'DISPUTED' ? (
                <TouchableOpacity style={[styles.ocBtnMobile, { backgroundColor: T.tertiary + '30', flex: 2 }]} disabled>
              ...
              >
                <Ionicons name="chatbubble-ellipses-outline" size={14} color={T.primary} style={{ marginRight: 4 }} />
  ...
  const renderFinance = () => (
    <View style={styles.tabContent}>
      ...
      <View style={styles.txListMobile}>
        {walletTransactions.map((tx) => (
          <View key={tx.id} style={styles.txItemMobile}>
            <View style={styles.txIconBoxMobile}>
              <Ionicons name={tx.type === 'debit' ? 'arrow-down-outline' : 'arrow-up-outline'} size={16} color={tx.type === 'debit' ? T.tertiary : T.primary} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.tableName}>{tx.description}</Text>
              <Text style={styles.txDateTxt}>{new Date(tx.created_at).toLocaleDateString()} â€¢ {tx.category}</Text>
            </View>
        ...
        {walletTransactions.length === 0 && <Text style={{ color: T.textSub, textAlign: 'center', padding: 20 }}>No transaction history</Text>}
      </View>
      </View>
    );
      ...
      <View style={styles.tabScrollWrap}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabScroller}>
          {[
            ...
            return (
              <TouchableOpacity key={link.id} style={[styles.tabItem, active && styles.tabItemActive]} onPress={() => setActiveTab(link.id)}>
                <Ionicons name={(active ? link.icon : `${link.icon}-outline`) as any} size={16} color={active ? T.primary : T.textSub} />
      ...
      <Modal visible={showProductModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.imagePickerContainer}>
              {(selectedImage || editingProduct?.image_url) ? (
                <View style={styles.imagePreviewContainer}>
                  <Image source={{ uri: selectedImage?.uri || editingProduct?.image_url }} style={styles.imagePreview} />
            ...
            <View style={styles.formGroup}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                {['Electronics', 'Fashion', 'Home', 'Beauty'].map(c => (
                  <TouchableOpacity
                    key={c}
                    style={[styles.smallBadge, newProduct.category === c && { backgroundColor: T.primary }]}
                    onPress={() => setNewProduct(p => ({ ...p, category: c }))}
                  >
                    <Text style={[styles.smallBadgeText, newProduct.category === c && { color: T.bg }]}>{c}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <View style={styles.formGroup}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                {['new', 'like-new', 'good', 'fair'].map(c => (
                  <TouchableOpacity
                    key={c}
                    style={[styles.smallBadge, newProduct.condition === c && { backgroundColor: T.primary }]}
                    onPress={() => setNewProduct(p => ({ ...p, condition: c }))}
                  >
                    <Text style={[styles.smallBadgeText, newProduct.condition === c && { color: T.bg }]}>{c.toUpperCase()}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <TouchableOpacity
              style={[styles.submitBtn, (uploading || (!selectedImage && !editingProduct)) && { opacity: 0.7 }]}
            ...
            >
              {uploading ? (
                ...
                <Text style={styles.submitBtnText}>{editingProduct ? 'Save Changes' : 'List Product'}</Text>
              )}
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  imagePickerBtn: { width: '100%', height: 120, borderRadius: 16, backgroundColor: T.surfaceHigh, borderStyle: 'dashed', borderWidth: 2, borderColor: T.primary + '40', alignItems: 'center', justifyContent: 'center' },
  imageIconCircle: { width: 56, height: 56, borderRadius: 28, backgroundColor: T.primary + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  imagePickerTxt: { color: T.textSub, fontSize: 13, fontWeight: '500' },
  imagePreviewContainer: { width: '100%', height: 180, borderRadius: 16, overflow: 'hidden', backgroundColor: T.surfaceHigh },
  imagePreview: { width: '100%', height: '100%', resizeMode: 'cover' },
  removeImageBtn: { position: 'absolute', top: 8, right: 8, backgroundColor: T.bg + 'CC', borderRadius: 12 },

  // Global Mobile Overrides
  container: { flex: 1, backgroundColor: T.bg },

  // Mobile Navbar
  navBarMobile: { height: 70, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, backgroundColor: T.surfaceLow, paddingTop: 10 },
  brandBoxMobile: { flexDirection: 'row', alignItems: 'center' },
  brandIconMobile: { width: 36, height: 36, borderRadius: 10, backgroundColor: T.primary + '20', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  brandNameMobile: { color: T.primary, fontSize: 16, fontWeight: '700' },
  brandSubtitleMobile: { color: T.textSub, fontSize: 9, fontWeight: '700', letterSpacing: 1, marginTop: 2 },
  navRight: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  navIconBtn: { padding: 4, position: 'relative' },
  notifDot: { width: 8, height: 8, backgroundColor: T.secondary, borderRadius: 4, position: 'absolute', top: 4, right: 4 },
  avatarBoxMobile: { width: 36, height: 36, borderRadius: 10, backgroundColor: T.surfaceHigh, alignItems: 'center', justifyContent: 'center' },

  // Scrollable Mobile Tabs
  tabScrollWrap: { borderBottomWidth: 1, borderBottomColor: T.border, backgroundColor: T.surfaceLow },
  tabScroller: { paddingHorizontal: 16, paddingVertical: 12, gap: 8 },
  tabItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, backgroundColor: T.surface },
  ...
  headerTitleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 },
  pageTitle: { fontSize: 28, fontWeight: '700', color: T.text },
  pageSubtitle: { fontSize: 13, color: T.textSub, marginTop: 4 },
  primaryButtonSolid: { flexDirection: 'row', alignItems: 'center', backgroundColor: T.primary, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
  iconButtonOutlined: { padding: 10, borderRadius: 8, borderWidth: 1, borderColor: T.border, alignItems: 'center', justifyContent: 'center' },
  ...
  statCard: { flexBasis: '48%', flexGrow: 1, backgroundColor: T.surfaceLow, borderWidth: 1, borderColor: T.border, borderRadius: 12, padding: 16 },
  ...
  chartContainer: { backgroundColor: T.surfaceLow, borderWidth: 1, borderColor: T.border, borderRadius: 12, padding: 16, marginBottom: 24 },
  chartHeader: { marginBottom: 16 },
  chartGraphArea: { height: 140, flexDirection: 'row', alignItems: 'flex-end', borderBottomWidth: 1, borderLeftWidth: 1, borderColor: T.border, padding: 8 },
  chartBars: { flex: 1, flexDirection: 'row', justifyContent: 'space-around', height: 120 },
  chartCol: { width: 1, backgroundColor: 'transparent', height: '100%', alignItems: 'center', justifyContent: 'flex-end' },
  chartBarLine: { width: 2, backgroundColor: T.primary, position: 'absolute', bottom: 0 },
  chartDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: T.primary, position: 'absolute' },
  chartDayLabel: { color: T.textSub, fontSize: 8, marginTop: 8, textTransform: 'uppercase', position: 'absolute', bottom: -20 },

  topSellingContainer: { backgroundColor: T.surfaceLow, borderWidth: 1, borderColor: T.border, borderRadius: 12, padding: 16 },
  ...
  searchBox: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: T.surfaceLow, paddingHorizontal: 12, borderRadius: 12, borderWidth: 1, borderColor: T.border },
  searchInput: { flex: 1, marginLeft: 8, height: 40, color: T.text, fontSize: 13 },

  productMobileCard: { backgroundColor: T.surfaceLow, borderRadius: 12, padding: 16, borderWidth: 1, borderColor: T.border },
  productMobileImg: { width: 56, height: 56, borderRadius: 8, backgroundColor: T.surfaceHigh },
  tableSku: { fontSize: 11, color: T.textSub },
  catBadge: { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, backgroundColor: T.primary + '15' },
  catBadgeText: { fontSize: 9, color: T.primary, fontWeight: '700', letterSpacing: 0.5 },
  stockBarBgMob: { width: 60, height: 6, backgroundColor: T.surfaceHigh, borderRadius: 3, overflow: 'hidden', marginLeft: 8 },
  stockBarFill: { height: '100%', borderRadius: 3 },
  stockText: { fontSize: 11, color: T.textSub, fontWeight: '600' },

  // Tab 3: Orders (Mobile)
  alertBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: T.tertiary + '15', borderWidth: 1, borderColor: T.tertiary + '30', borderLeftWidth: 4, borderLeftColor: T.tertiary, padding: 12, borderRadius: 12, marginBottom: 20 },
  alertIconBox: { width: 32, height: 32, borderRadius: 16, backgroundColor: T.tertiary + '20', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  alertTitle: { fontSize: 13, fontWeight: '700', color: T.tertiary, marginBottom: 2 },
  alertSub: { fontSize: 11, color: T.tertiary, opacity: 0.8 },

  orderMobileCard: { backgroundColor: T.surfaceLow, borderWidth: 1, borderColor: T.border, borderRadius: 12, padding: 16 },
  ...
  ocTinyLabel: { fontSize: 9, color: T.textSub, fontWeight: '700', letterSpacing: 1, marginBottom: 4 },
  ocBuyerName: { fontSize: 12, fontWeight: '700', color: T.text },
  ocAddressTxt: { fontSize: 11, color: T.textSub, marginTop: 2 },

  ocPinBoxMobile: { backgroundColor: T.surface, borderWidth: 1, borderColor: T.primary + '30', borderRadius: 6, paddingVertical: 4, paddingHorizontal: 8, alignSelf: 'flex-start' },
  ...
  ocBtnMobile: { paddingVertical: 10, borderRadius: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'center' },
  ocBtnTxt: { color: T.bg, fontSize: 13, fontWeight: '700' },
  ocBtnMobileOutlined: { paddingVertical: 10, borderRadius: 8, alignItems: 'center', borderWidth: 1, borderColor: T.border },
  ocBtnOutlinedTxtMobile: { color: T.textSub, fontSize: 13, fontWeight: '600' },

  // Tab 4: Finance (Mobile)
  financeHeroMobile: { backgroundColor: T.primaryDark, borderRadius: 12, padding: 20, overflow: 'hidden' },
  ...
  fhTrend: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.1)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  fhTrendTxt: { color: T.bg, fontSize: 11, fontWeight: '600', marginLeft: 6 },
  fhWithdrawBtn: { backgroundColor: T.bg, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
  fhWithdrawTxt: { color: T.primaryDark, fontSize: 12, fontWeight: '700' },

  txListMobile: { backgroundColor: T.surfaceLow, borderRadius: 12, borderWidth: 1, borderColor: T.border, padding: 16, gap: 16 },
  txItemMobile: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  txIconBoxMobile: { width: 36, height: 36, borderRadius: 8, backgroundColor: T.surfaceHigh, alignItems: 'center', justifyContent: 'center' },
  ...
  modalContent: { backgroundColor: T.surfaceLow, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 },
  modalTitle: { fontSize: 20, fontWeight: '700', color: T.text },
  formGroup: { marginBottom: 20 },
  inputLabel: { fontSize: 12, color: T.textSub, fontWeight: '600', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 },
  modalInput: { backgroundColor: T.surface, borderRadius: 12, borderWidth: 1, borderColor: T.border, padding: 12, color: T.text, fontSize: 15 },
  smallBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: T.surfaceHigh },
  smallBadgeText: { fontSize: 12, color: T.text, fontWeight: '600' },
  submitBtn: { backgroundColor: T.primary, paddingVertical: 16, borderRadius: 12, alignItems: 'center', marginTop: 10 },
  submitBtnText: { color: T.bg, fontSize: 16, fontWeight: '700' },
  deleteBtnSmall: { padding: 6, borderRadius: 8, backgroundColor: T.surfaceHigh, borderWidth: 1, borderColor: T.border },
  
  // Shipment Success Specific
  successCirc: { width: 100, height: 100, borderRadius: 50, alignItems: 'center', justifyContent: 'center' },
  pinDisplayCard: { backgroundColor: T.surface, borderWidth: 1, borderColor: T.primary + '30', borderRadius: 16, padding: 24, marginTop: 24, width: '100%', alignItems: 'center' },

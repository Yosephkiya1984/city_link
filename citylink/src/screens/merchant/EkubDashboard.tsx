      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            const res = await performEkubDraw(circle.id, circle.current_round, eligibleIds, circle.pot_balance);
            if (!res.error) {
      ...
      >
        {loading ? (
          ...
          <View style={{ gap: 16 }}>
            {activeTab === 'circles' && (
              <>
                <Text style={{ color: C.text, fontSize: 18, fontFamily: Fonts.bold }}>My Circles</Text>
                {circles.map(c => (
            ...
            {activeTab === 'draws' && (
              <>
                <Text style={{ color: C.text, fontSize: 18, fontFamily: Fonts.bold }}>Pending Payouts</Text>
                {activeDraws.map(draw => (
                  <PayoutReleaseCard key={draw.id} draw={draw} onRelease={() => onReleasePayout(draw.id)} C={C} />
                ))}
                {activeDraws.length === 0 && <EmptyState text="No payouts awaiting release" C={C} />}
              </>
            )}
          </View>
        )}
...
const CircleManagerCard = ({ circle, onRunDraw, C }: { circle: EkubCircle, onRunDraw: () => void, C: any }) => (
  <View style={{ backgroundColor: C.surface, borderRadius: Radius.xl, padding: 16, borderLeftWidth: 4, borderLeftColor: C.primary, ...Shadow.md }}>
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
      <View>
        <Text style={{ color: C.text, fontSize: 16, fontFamily: Fonts.bold }}>{circle.name}</Text>
        <Text style={{ color: C.sub, fontSize: 12 }}>Round {circle.current_round} â€¢ {circle.frequency}</Text>
      </View>
      <View style={{ alignItems: 'flex-end' }}>
        <Text style={{ color: C.primary, fontSize: 16, fontFamily: Fonts.bold }}>ETB {circle.pot_balance.toLocaleString()}</Text>
...
const PayoutReleaseCard = ({ draw, onRelease, C }: { draw: any, onRelease: () => void, C: any }) => (
  <View style={{ backgroundColor: C.surface, borderRadius: Radius.xl, padding: 16, borderLeftWidth: 4, borderLeftColor: C.amber, ...Shadow.md }}>
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
      <View>
        <Text style={{ color: C.text, fontSize: 16, fontFamily: Fonts.bold }}>Payout Release</Text>
        <Text style={{ color: C.sub, fontSize: 12 }}>{draw.ekubs?.name} â€¢ Round {draw.round_number}</Text>
        <Text style={{ color: C.primary, fontSize: 13, marginTop: 4 }}>Winner: {draw.winner_name}</Text>
      </View>
      <View style={{ alignItems: 'flex-end' }}>
        <Text style={{ color: C.amber, fontSize: 16, fontFamily: Fonts.bold }}>ETB {draw.pot_amount.toLocaleString()}</Text>
...
const AppReviewCard = ({ app, onReview, C }: { app: any, onReview: (s: string) => void, C: any }) => (
  <View style={{ backgroundColor: C.surface, borderRadius: Radius.xl, padding: 16, ...Shadow.md }}>
    <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
      {app.user?.profile_image ? (
        <Image source={{ uri: app.user.profile_image }} style={{ width: 40, height: 40, borderRadius: 20 }} />
      ) : (
        <Ionicons name="person-circle" size={40} color={C.sub} />
      )}
      <View style={{ flex: 1 }}>
        <Text style={{ color: C.text, fontSize: 15, fontFamily: Fonts.bold }}>{app.user?.full_name || 'Anonymous User'}</Text>
        <Text style={{ color: C.sub, fontSize: 12 }}>Applying for: {app.ekubs?.name}</Text>
        <Text style={{ color: C.text, fontSize: 13, marginTop: 4 }}>"{app.application_reason}"</Text>
      </View>
    </View>
    <View style={{ flexDirection: 'row', gap: 8 }}>
      <CButton title="Reject" size="sm" variant="ghost" style={{ flex: 1 }} onPress={() => onReview('REJECTED')} />
      <CButton title="Approve Member" size="sm" style={{ flex: 2 }} onPress={() => onReview('ACTIVE')} />
    </View>
  </View>
);

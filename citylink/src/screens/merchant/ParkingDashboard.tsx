...
import { fetchParkingSessions, fetchParkingLots, updateSessionStatus, updateParkingLot } from '../../services/parking.service';
...
export default function ParkingDashboard() {
  ...
  const todayRevenue = sessions
    .filter(s => s.status === 'COMPLETED' && new Date(s.end_time).toDateString() === new Date().toDateString())
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (sessionsRes.data) {
        const sortedSessions = sessionsRes.data.sort((a, b) => new Date(b.start_time).getTime() - new Date(a.start_time).getTime());
  ...
  const getSpotStatus = (spotNumber) => {
    const session = sessions.find(s => 
      s.lot_id === selectedLot?.id && 
      s.spot_id === spotNumber && 
      s.status === 'ACTIVE'
    );
  ...
  const getSpotColor = (status) => {
    switch (status) {
      case 'available': return PARKING_COLORS.spot.available;
      case 'occupied': return PARKING_COLORS.spot.occupied;
      case 'blocked': return PARKING_COLORS.spot.blocked;
      default: return PARKING_COLORS.spot.available;
    }
  };
  ...
  const SessionCard = ({ session }) => {
    const elapsed = session.start_time ? 
      Math.floor((new Date().getTime() - new Date(session.start_time).getTime()) / 60000) : 0;
        ...
        <Card style={{ marginBottom: 8, padding: 12 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <Text style={{ color: C.text, fontSize: 14, fontFamily: Fonts.black }}>
                  {selectedLot?.spot_prefix || ''}{session.spot_id}
                </Text>
                <View style={{ 
                  paddingHorizontal: 6, 
                  paddingVertical: 2, 
                  borderRadius: 4, 
                  backgroundColor: session.status === 'ACTIVE' ? PARKING_COLORS.primaryL : 'rgba(0,168,107,0.1)' 
                }}>
                  <Text style={{ 
            ...
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={{ color: PARKING_COLORS.primary, fontSize: 14, fontFamily: Fonts.black }}>
                {fmtETB(currentFare)}
              </Text>
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <View>
                ...
                <Text style={{ color: PARKING_COLORS.primary, fontSize: 32, fontFamily: Fonts.black, marginTop: 4 }}>
                  {occupancyRate}%
                </Text>
              </View>
              <View style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: PARKING_COLORS.primaryL, alignItems: 'center', justifyContent: 'center' }}>
                <Ionicons name="car" size={24} color={PARKING_COLORS.primary} />
              </View>
            </View>
            
            <View style={{ height: 1, backgroundColor: 'rgba(45,126,240,0.2)', marginVertical: 20 }} />
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
              ...
              <View style={{ alignItems: 'center' }}>
                ...
                <Text style={{ color: 'rgba(232,49,42,0.7)', fontSize: 10, fontFamily: Fonts.bold }}>
                  OCCUPIED
                </Text>
              </View>
              <View style={{ alignItems: 'center' }}>
                <Text style={{ color: PARKING_COLORS.primary, fontSize: 16, fontFamily: Fonts.black }}>
          ...
          <View>
            ...
            <View style={{ 
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <View style={{ width: 12, height: 12, borderRadius: 2, backgroundColor: '#00A86B' }} />
                <Text style={{ color: C.sub, fontSize: 10 }}>Available</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <View style={{ width: 12, height: 12, borderRadius: 2, backgroundColor: '#E8312A' }} />
                <Text style={{ color: C.sub, fontSize: 10 }}>Occupied</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <View style={{ width: 12, height: 12, borderRadius: 2, backgroundColor: '#8A9AB8' }} />
        ...
        {activeTab === 'sessions' && (
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {loading && sessions.length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>Loading sessions...</Text>
            ) : sessions.filter(s => s.status === 'ACTIVE').length === 0 ? (
              <Text style={{ color: C.sub, textAlign: 'center', padding: 20 }}>No active sessions</Text>
            ) : (
              sessions
            ...
            {exitRequests.length > 0 && (
              <View style={{ marginTop: 20 }}>
                ...
                {exitRequests.map(session => (
                  <Card key={session.id} style={{ marginBottom: 8, padding: 12, backgroundColor: 'rgba(245,184,0,0.05)' }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                      <View>
                        <Text style={{ color: C.text, fontSize: 14, fontFamily: Fonts.black }}>
                          Spot {selectedLot?.spot_prefix || ''}{session.spot_id}
                        </Text>
                        <Text style={{ color: C.sub, fontSize: 11 }}>
              ...
              <Card style={{ padding: 16, marginBottom: 16 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <Text style={{ color: C.text, fontSize: 16, fontFamily: Fonts.black }}>
                    Spot {selectedLot?.spot_prefix || ''}{selectedSession.spot_id}
                  </Text>
                  <View style={{ 
                    paddingHorizontal: 8, 
                    paddingVertical: 4, 
                    borderRadius: 6, 
                    backgroundColor: selectedSession.status === 'ACTIVE' ? PARKING_COLORS.primaryL : 'rgba(0,168,107,0.1)' 
                  }}>
                    <Text style={{ 
                      color: selectedSession.status === 'ACTIVE' ? PARKING_COLORS.primary : '#00A86B', 
                      fontSize: 10, 
                      fontFamily: Fonts.bold,
                      textTransform: 'uppercase' 
                    }}>
                ...
                <View style={{ marginBottom: 8 }}>
                  ...
                  <Text style={{ color: PARKING_COLORS.primary, fontSize: 18, fontFamily: Fonts.black }}>

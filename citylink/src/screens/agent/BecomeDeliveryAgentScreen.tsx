...
export default function BecomeDeliveryAgentScreen() {
  ...
  const handleSubmit = async () => {
    if (!vehicleType) { showToast('Select a vehicle type', 'error'); return; }
    if (!licenseNumber || licenseNumber.trim().length < 5) {
      showToast('Enter a valid license / ID number', 'error'); return;
    }
    if (needsPlate && !plateNumber.trim()) {
      showToast('Enter your vehicle plate number', 'error'); return;
    }
    ...
    try {
      ...
      if (!granted) {
        Alert.alert('Location Required', 'CityLink Delivery needs your location to dispatch orders to you. Please enable location access.');
        setLoading(false);
        return;
      }
      ...
      showToast(existing ? "Welcome back! Redirecting to your dashboard." : "Application submitted! You'll be notified once approved.", 'success');
      navigation.goBack();
    } catch (e) {
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={['#1a2a1a', '#1f3a1f']} style={s.earningsBanner}>
          ...
          <View style={{ flex: 1, marginLeft: 14 }}>
            ...
            <Text style={s.earningsSub}>On a 500 ETB order, you earn 60 ETB straight to your CityLink Wallet</Text>
          </View>
        </LinearGradient>
          ...
          <View style={s.vehicleGrid}>
            {VEHICLE_TYPES.map(v => (
              <TouchableOpacity
                ...
                onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setVehicleType(v.value); }}
              >
        ...
        <View style={s.infoBox}>
          ...
          <Text style={s.infoText}>
            Your application will be reviewed by the CityLink admin team within 24 hours.
            You'll receive a notification once approved and can start accepting deliveries immediately.
          </Text>
        </View>
        ...
        >
          {loading ? (
            ...
            <>
              <Ionicons name="rocket-outline" size={20} color="#0d1117" style={{ marginRight: 8 }} />
...
const s = StyleSheet.create({
  ...
  backBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: T.surface, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  headerCenter: { flex: 1 },
  ...
  earningsBanner: { borderRadius: 16, padding: 18, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(104,211,145,0.2)' },
  earningsTitle: { color: T.green, fontSize: 16, fontWeight: '800' },
  earningsSub: { color: T.textSub, fontSize: 12, marginTop: 4, lineHeight: 18 },

  section: { backgroundColor: T.surface, borderRadius: 16, padding: 18, borderWidth: 1, borderColor: T.border },
  ...
  vehicleCard: {
    width: (width - 76) / 3, backgroundColor: T.card, borderRadius: 12, padding: 12,
    alignItems: 'center', borderWidth: 1, borderColor: T.border, position: 'relative',
  },
  ...
  vehicleCheck: { position: 'absolute', top: 6, right: 6, width: 18, height: 18, borderRadius: 9, backgroundColor: T.primary, alignItems: 'center', justifyContent: 'center' },

  inputGroup: { marginBottom: 14 },
  label: { color: T.textSub, fontSize: 12, fontWeight: '600', marginBottom: 8, letterSpacing: 0.5 },
  inputWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: T.surfaceHigh, borderRadius: 10, borderWidth: 1, borderColor: T.border },
  inputIcon: { marginLeft: 12 },
  input: { flex: 1, color: T.text, padding: 12, fontSize: 14 },

  infoBox: { flexDirection: 'row', gap: 10, backgroundColor: 'rgba(99,179,237,0.06)', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: 'rgba(99,179,237,0.15)' },
  infoText: { flex: 1, color: T.textSub, fontSize: 12, lineHeight: 18 },

  submitBtn: { backgroundColor: T.primary, borderRadius: 14, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },

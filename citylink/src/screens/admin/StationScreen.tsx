    ...
    <View style={{ flex:1, backgroundColor:C.ink }}>
      ...
      <View style={{ backgroundColor:C.surface, borderBottomWidth:1, borderBottomColor:C.edge,
        ...
        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center' }}>
          <View>
            <Text style={{ color:C.text, fontSize:FontSize.xl, fontWeight:'800' }}>Station Operator</Text>
          ...
          <View style={{ flexDirection:'row', gap:12, alignItems:'center' }}>
            ...
            {/* Live indicator */}
            <View style={{ flexDirection:'row', alignItems:'center', gap:6, backgroundColor:C.greenL,
              borderRadius:20, paddingHorizontal:10, paddingVertical:4,
              borderWidth:1, borderColor:C.greenB }}>
      ...
      <View style={{ flexDirection:'row', borderBottomWidth:1, borderBottomColor:C.edge }}>
        {[
          ...
          <View key={label} style={{ flex:1, alignItems:'center', padding:10 }}>
            ...
            <Text style={{ color:C.sub, fontSize:FontSize.xs, marginTop:1, textAlign:'center' }}>{label}</Text>
          </View>
        ))}
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom:40 }} showsVerticalScrollIndicator={false}>
        ...
        {/* â”€â”€ GATE EVENTS â”€â”€ */}
        {activeTab === 'gate' && events.map((ev) => (
          <View key={ev.id} style={{ flexDirection:'row', alignItems:'center',
            paddingHorizontal:16, paddingVertical:10,
            borderBottomWidth:1, borderBottomColor:C.edge,
            ...
            <View style={{ flex:1 }}>
              <View style={{ flexDirection:'row', alignItems:'center', gap:6 }}>
                <Text style={{ color:C.text, fontWeight:'700', fontSize:FontSize.md }}>{ev.user}</Text>
                {!ev.valid && (
            ...
            <View style={{ alignItems:'flex-end' }}>
              ...
              <Text style={{ color:C.sub, fontSize:FontSize.xs, marginTop:2 }}>
                {new Date(ev.time).toLocaleTimeString('en-ET', { hour:'2-digit', minute:'2-digit' })}
              </Text>
            </View>
          </View>
        ))}
        ...
        {activeTab === 'stats' && (
          <View style={{ padding:16, gap:12 }}>
            <View style={{ backgroundColor:C.surface, borderRadius:Radius.xl, borderWidth:1,
              borderColor:C.edge, padding:16 }}>
              <Text style={{ color:C.text, fontWeight:'700', marginBottom:16 }}>Hourly Throughput</Text>
              <View style={{ flexDirection:'row', alignItems:'flex-end', gap:4, height:80 }}>
                {[30,55,80,70,90,65,45,60,75,85,70,50].map((pct, i) => (
                  <View key={i} style={{ flex:1 }}>
                    <View style={{ height:pct*0.72, backgroundColor:C.greenL,
                      borderRadius:2, borderWidth:1, borderColor:C.greenB }} />
            ...
            {[
              ...
              <View key={label} style={{ flexDirection:'row', alignItems:'center',
                ...
                borderColor:C.edge, padding:16, gap:14 }}>
                <Text style={{ fontSize:22 }}>{icon}</Text>
                <View style={{ flex:1 }}>
                  <Text style={{ color:C.sub, fontSize:FontSize.xs }}>{label}</Text>
                  <Text style={{ color:C.text, fontWeight:'800', fontSize:FontSize.xl }}>{value}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        {activeTab === 'trains' && (
          <View style={{ padding:16, gap:10 }}>
            {TRAINS.map((train) => (
              <View key={train.id} style={{ backgroundColor:C.surface, borderRadius:Radius.xl,
                borderWidth:1, borderColor:C.edge, padding:16, flexDirection:'row', alignItems:'center', gap:14 }}>
                <View style={{ width:40, height:40, borderRadius:12,
                  backgroundColor:train.line==='NS'?C.greenL:C.blueL,
                  alignItems:'center', justifyContent:'center' }}>
                  <Text style={{ color:train.line==='NS'?C.green:C.blue, fontWeight:'800' }}>{train.id}</Text>
                </View>
                <View style={{ flex:1 }}>
                  <View style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
                    <Text style={{ color:C.text, fontWeight:'700' }}>Line {train.line}</Text>
                    <View style={{ paddingHorizontal:6, paddingVertical:2, borderRadius:4,
                      backgroundColor:train.status==='AT_STATION'?C.greenL
                        :train.status==='ARRIVING'?C.amberL:C.blueL }}>
                      <Text style={{ fontSize:FontSize.xs, fontWeight:'700',
                        color:train.status==='AT_STATION'?C.green
                          :train.status==='ARRIVING'?C.amber:C.blue }}>
                        {train.status.replace('_',' ')}
                      </Text>
                    </View>
                  </View>
                  <Text style={{ color:C.sub, marginTop:2 }}>ðŸ“ {train.station}</Text>
                  <Text style={{ color:C.sub, fontSize:FontSize.xs }}>👥 {train.passengers} passengers</Text>
                </View>
                {train.delay > 0 && (

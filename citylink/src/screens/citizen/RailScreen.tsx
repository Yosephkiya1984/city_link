...
const TABS = [
  { value: 'lrt', label: 'ðŸš‡ LRT' },
  { value: 'edr', label: 'ðŸš‚ EDR' },
];
...
export default function RailScreen() {
  ...
  async function handleTapIn(station) {
    ...
    if (!currentUser) {
      showToast('Sign in to tap in', 'error');
      return;
    }
    if (!origin) {
      showToast('Select your boarding station', 'error');
      return;
    }
    if (balance < 10) {
      showToast('Minimum 10 ETB balance required to travel', 'error');
      return;
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink }}>
        ...
        {tab === 'lrt' && (
          <>
            ...
            {lrtJourney && (
              <View
                style={{
                  ...
                  backgroundColor:
                    lrtJourney.status === 'completed'
                      ? 'rgba(45,126,240,0.06)'
                      : 'rgba(0,168,107,0.06)',
                }}
              >
                <View
                ...
                >
                  <Text
                    style={{
                      color: lrtJourney.status === 'completed' ? C.blue : C.green,
            ...
            >
                ...
                return (
                  <View
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      paddingVertical: 8,
                      borderBottomWidth: i === LRT_STATIONS.NS.length - 1 ? 0 : 1,
                      borderBottomColor: C.edge + '40',
                    }}
                  >
                    <View
                      style={{
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {trainAt && (
                        <Text
                          style={{ color: C.green, fontSize: 10, fontWeight: '700', marginTop: 2 }}
                        >
              ...
              >
                <Text style={{ color: C.blue, fontSize: FontSize.sm, fontWeight: '700' }}>
                  ðŸ”µ Eastâ€“West Line
                </Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                  <View
                    style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: C.green }}
                ...
                return (
                  <View
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      paddingVertical: 8,
                      borderBottomWidth: i === LRT_STATIONS.EW.length - 1 ? 0 : 1,
                      borderBottomColor: C.edge + '40',
                    }}
                  >
                    <View
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {trainAt && (
                        <Text
                          style={{ color: C.blue, fontSize: 10, fontWeight: '700', marginTop: 2 }}
                        >
                          ðŸšŠ {trainAt.dir} ({Math.round(trainAt.occupancy * 100)}% full)
                        </Text>
                      )}
            ...
            <View
              style={{
                marginHorizontal: 16,
                backgroundColor: 'rgba(10,30,18,1)',
                borderRadius: Radius.xl,
                borderWidth: 1,
                borderColor: 'rgba(0,168,107,0.2)',
                padding: 16,
                      ...
                      {allStations.map((s) => (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            borderWidth: 1,
                            borderColor: selectedOrigin?.id === s.id ? C.green : C.edge2,
                          }}
                        >
                          <Text
                            style={{
                              ...
                              fontWeight: '600',
                              color: selectedOrigin?.id === s.id ? C.green : C.sub,
                            }}
                          >
                            {s.name}
                          </Text>
          ...
          <>
            ...
            >
              <Text style={{ color: C.sub, fontSize: FontSize.md, lineHeight: 20 }}>
                ðŸš‚ Book intercity rail tickets on the Ethio-Djibouti Railway connecting Addis
                Ababa to Djibouti City.
              </Text>
            </View>
                ...
                <View style={{ padding: 14, backgroundColor: 'rgba(26,18,0,0.8)' }}>
                  ...
                  >
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      <Text
                        style={{ color: C.amber, fontSize: FontSize['2xl'], fontWeight: '800' }}
                      >
      ...
      <Modal visible={tapModal} transparent animationType="slide">
        ...
        >
          ...
          {tapType === 'out' && selectedExit && (
            <CButton
              title={`Tap Out â†’ ${fmtETB(
                calcLrtFare(
                  allStations.find((s) => s.id === lrtJourney?.originId)?.km || 0,
                  selectedExit.km
                )
              )} ETB`}
      ...
      <Modal visible={edrBookingModal} transparent animationType="slide">
        <Pressable
          style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)' }}
          onPress={() => setEdrBookingModal(false)}
        />
        <View
          style={{
            backgroundColor: C.surface,
        ...
        >
          {selectedTrain && (
            <>
              ...
              <CButton
                title={`Book â€” ${fmtETB(selectedTrain.price, 0)} ETB`}
                onPress={() => handleEdrBook(selectedTrain)}
                style={{ marginTop: 20 }}
              />
              <CButton
                title="Cancel"

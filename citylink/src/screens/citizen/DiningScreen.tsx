...
export function DiningScreen() {
  ...
  async function handleReservation() {
    if (!currentUser) { showToast(t('Sign in to reserve', lang), 'error'); return; }
    ...
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <View key={r.id} style={{ backgroundColor: C.surface, borderRadius: Radius.xl, borderWidth: 1,
              borderColor: C.edge, padding: 16, marginBottom: 10 }}>
            ...
            <CButton title={t('Reserve Table', lang)} onPress={() => setReserveModal(r)}
      ...
      <Modal visible={!!reserveModal} transparent animationType="slide">
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} onPress={() => setReserveModal(null)}>
          <Pressable style={{ backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 }} onPress={e => e.stopPropagation()}>
            ...
            <CButton title={t('Confirm Reservation', lang)} onPress={handleReservation} style={{ marginTop: 8 }} />
            <CButton title={t('Cancel', lang)} onPress={() => setReserveModal(null)} variant="ghost" style={{ marginTop: 8 }} />
          </Pressable>
        </Pressable>
      </Modal>
    </View>

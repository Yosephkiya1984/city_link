...
export default function FaydaKYCScreen() {
  ...
  const getStatusInfo = (status) => {
    const statusMap = {
      [FAYDA_STATUS.NOT_STARTED]: { color: C.hint, icon: 'document-text-outline', label: 'Not Started' },
      [FAYDA_STATUS.INITIATED]: { color: C.amber, icon: 'time-outline', label: 'In Progress' },
      [FAYDA_STATUS.PENDING_VERIFICATION]: { color: C.amber, icon: 'hourglass-outline', label: 'Pending' },
  ...
  const testWithMockData = (mockId) => {
    ...
    if (mockRecord) {
      ...
      showToast(`Loaded test data for ${mockRecord.full_name}\nPhone: ${mockRecord.phone}\nDOB: ${mockRecord.date_of_birth}\nRegion: ${mockRecord.region}`, 'success');
    }
  };
        ...
        <Card style={{ marginBottom: 20 }}>
          ...
          {kycData && (
            <View style={{ 
              ...
              <Text style={{ color: C.green, fontSize: 12, fontFamily: Fonts.medium, textAlign: 'center' }}>
                âœ“ Verified: {kycData.full_name}
              </Text>
              <Text style={{ color: C.green, fontSize: 11, fontFamily: Fonts.regular, textAlign: 'center' }}>
          ...
          <View>
            ...
            {kycStatus === FAYDA_STATUS.INITIATED && (
              <View style={{
                ...
                <Text style={{ color: C.amber, fontSize: 14, fontFamily: Fonts.medium, textAlign: 'center' }}>
                  Your KYC process has been initiated. Please visit the nearest Fayda registration center to complete biometric verification.
                </Text>
              </View>
            )}
          </View>
        ...
        {activeTab === 'verification' && (
          <View>
            ...
            {!showVerificationForm ? (
              <View>
                <Text style={{ color: C.sub, fontSize: 14, fontFamily: Fonts.medium, marginBottom: 20, textAlign: 'center' }}>
                ...
                <View style={{ marginBottom: 20 }}>
                  <Text style={{ color: C.hint, fontSize: 12, fontFamily: Fonts.medium, marginBottom: 8, textAlign: 'center' }}>
                    Test with mock data (development only):
                  </Text>
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    {Object.keys(FAYDA_MOCK_DATABASE).slice(0, 3).map((mockId) => (
                      <TouchableOpacity
                        key={mockId}
                  ...
                  <View style={{ marginTop: 12, padding: 12, backgroundColor: C.amber + '10', borderRadius: Radius.lg, borderWidth: 1, borderColor: C.amber + '30' }}>
                    <Text style={{ color: C.amber, fontSize: 11, fontFamily: Fonts.medium, textAlign: 'center' }}>
                    ...
                    {Object.keys(FAYDA_MOCK_DATABASE).map((mockId, index) => {
                      ...
                      return (
                        <View key={mockId} style={{ marginTop: 4 }}>
                          <Text style={{ color: C.amber, fontSize: 9, fontFamily: Fonts.regular, textAlign: 'center' }}>
                            {index + 1}. {mockId} - {mockRecord.full_name}
                          </Text>
                          <Text style={{ color: C.amber, fontSize: 8, fontFamily: Fonts.regular, textAlign: 'center', opacity: 0.8 }}>
                            ðŸ“± {mockRecord.phone} | ðŸŽ‚ {mockRecord.date_of_birth} | ðŸ“ {mockRecord.region}
                          </Text>
                        </View>
                      );
                    })}

    ...
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          {CONTACTS.map((c, idx) => (
            <TouchableOpacity key={idx} onPress={() => dialNumber(c.num)}
              style={{ width: '48%', backgroundColor: C.surface, borderRadius: Radius.xl, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: C.edge, ...Shadow.sm }}>
               <View style={{ width: 56, height: 56, borderRadius: 28, backgroundColor: c.color + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                  <Ionicons name={c.icon} size={32} color={c.color} />
               </View>
               <Text style={{ color: C.text, fontFamily: Fonts.black, fontSize: 16 }}>{c.name}</Text>
               <Text style={{ color: c.color, fontFamily: Fonts.black, fontSize: 24, marginTop: 4 }}>{c.num}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>

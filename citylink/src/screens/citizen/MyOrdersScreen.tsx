...
import { 
  ...
  Animated, StyleSheet, Dimensions, Modal, TextInput, ActivityIndicator, Alert
} from 'react-native';
...
const getStatusStyle = (status, C) => {
  switch (status) {
    case 'PAID':           
    case 'ORDER_PLACED':   return { bg: C.primary + '15',  text: C.primary,   border: C.primary + '40' };
    case 'DISPATCHING':    
    case 'PREPARING':      return { bg: '#60a5fa15',        text: '#60a5fa',   border: '#60a5fa40' };
    case 'AGENT_ASSIGNED': return { bg: '#818cf815',        text: '#818cf8',   border: '#818cf840' };
    case 'SHIPPED':
    case 'IN_TRANSIT':     
    case 'OUT_FOR_DELIVERY': return { bg: '#f59e0b15',        text: '#f59e0b',   border: '#f59e0b40' };
    case 'AWAITING_PIN':   return { bg: '#10b98115',        text: '#10b981',   border: '#10b98140' };
    case 'COMPLETED':      
    case 'DELIVERED':      return { bg: C.primary + '15',   text: C.primary,   border: C.primary + '40' };
    case 'CANCELLED':      return { bg: '#64748b15',        text: '#64748b',   border: '#64748b40' };
    default:               return { bg: C.surface,          text: C.sub,       border: C.edge };
  }
};
...
const EscrowStatus = ({ order, C }) => {
  const config = {
    PAID:           { icon: 'lock-closed', text: 'Funds locked in escrow â€” safe until delivery', color: '#f59e0b' },
    DISPATCHING:    { icon: 'search',      text: 'Searching for a nearby delivery partner', color: '#60a5fa' },
    AGENT_ASSIGNED: { icon: 'person',      text: 'Delivery agent assigned and heading to merchant', color: '#818cf8' },
    IN_TRANSIT:     { icon: 'bicycle',     text: 'Agent has picked up your order and is on the way', color: '#f59e0b' },
    AWAITING_PIN:   { icon: 'home',        text: 'Agent is at your location â€” provide PIN to finish', color: '#10b981' },
    SHIPPED:        { icon: 'bicycle',     text: 'Merchant has shipped the order', color: C.primary },
    COMPLETED:      { icon: 'checkmark-circle', text: 'Escrow released and order completed', color: '#8b5cf6' },
  ...
  return (
    <View style={[styles.escrowRow, { backgroundColor: c.color + '12', borderColor: c.color + '30' }]}>
      ...
      >
        ...
        <View style={styles.cardHeader}>
          <View style={[styles.orderIcon, { backgroundColor: statusStyle.bg }]}>
            <Ionicons name={order.type === 'food' ? "restaurant-outline" : "bag-handle-outline"} size={22} color={statusStyle.text} />
          </View>
          <View style={styles.headerText}>
            <Text style={[styles.productName, { color: C.text }]} numberOfLines={1}>
              {order.type === 'food' ? (order.items_json?.[0]?.name || 'Food Order') : (order.product_name || 'Product')}
            </Text>
            <Text style={[styles.orderId, { color: C.sub }]}>
              #{order.id?.slice(0, 8).toUpperCase()} Â· {order.type === 'food' ? `${order.items_count || 1} items` : `Qty ${order.quantity || 1}`}
            </Text>
            {order.type === 'food' && (
              <Text style={{ fontFamily: Fonts.bold, fontSize: 10, color: C.primary, marginTop: 2 }}>
        ...
        {order.status === 'PAID' && (
          <View style={[styles.countdownRow, { backgroundColor: '#f59e0b10', borderColor: '#f59e0b25' }]}>
        ...
        {['DISPATCHING', 'AGENT_ASSIGNED', 'SHIPPED', 'IN_TRANSIT', 'AWAITING_PIN'].includes(order.status) && (
            ...
            <View style={styles.trackerSteps}>
               {['Paid', 'Dispatched', 'On Route', 'Arrived'].map((step, idx) => {
                 ...
                 if (idx === 0) {
                   ...
                   isPassed = ['DISPATCHING', 'AGENT_ASSIGNED', 'SHIPPED', 'IN_TRANSIT', 'AWAITING_PIN', 'COMPLETED'].includes(order.status);
                 } else if (idx === 2) {
                   isActive = ['SHIPPED', 'IN_TRANSIT'].includes(order.status);
                   isPassed = ['SHIPPED', 'IN_TRANSIT', 'AWAITING_PIN', 'COMPLETED'].includes(order.status);
                 } else if (idx === 3) {
                 ...
                 return (
                   <View key={idx} style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                     <View style={[styles.trackerDot, isPassed ? { backgroundColor: C.primary, borderColor: C.primary } : { borderColor: C.edge, backgroundColor: C.ink }]} />
                     {idx < 3 && <View style={[styles.trackerLine, isPassed && !isActive ? { backgroundColor: C.primary } : { backgroundColor: C.edge }]} />}
                   </View>
                 );
               })}
            </View>
            <View style={styles.trackerLabels}>
              <Text style={[styles.trackerLabelText, { color: C.text, textAlign: 'left' }]}>Paid</Text>
              <Text style={[styles.trackerLabelText, { color: C.text, textAlign: 'center' }]}>Dispatched</Text>
              <Text style={[styles.trackerLabelText, { color: C.text, textAlign: 'center' }]}>On Route</Text>
              <Text style={[styles.trackerLabelText, { color: C.text, textAlign: 'right' }]}>Arrived</Text>
            </View>
              ...
              >
                <View>
                  <Text style={[styles.pinBoxTitle, { color: C.primary }]}>
                    {order.status === 'AWAITING_PIN' ? 'âš ï¸ Agent arrived! Give this PIN to driver:' : 'Delivery PIN (Keep safe)'}
                  </Text>
        ...
        <View style={styles.cardFooter}>
          ...
          {['SHIPPED', 'AWAITING_PIN', 'DISPATCHING', 'AGENT_ASSIGNED', 'IN_TRANSIT'].includes(order.status) ? (
...
export default function MyOrdersScreen() {
  ...
  useEffect(() => {
    loadOrders();
    const ch = subscribeToTable(`buyer-orders-${user?.id}`, 'marketplace_orders', `buyer_id=eq.${user?.id}`, (payload) => {
      if (payload.eventType === 'UPDATE') {
        setMktOrders(prev => prev.map(o => o.id === payload.new.id ? { ...o, ...payload.new, type: 'marketplace' } : o));
      } else if (payload.eventType === 'INSERT') {
        setMktOrders(prev => [{ ...payload.new, type: 'marketplace' }, ...prev]);
      }
    });

    const foodCh = subscribeToTable(`buyer-food-${user?.id}`, 'food_orders', `citizen_id=eq.${user?.id}`, (payload) => {
      if (payload.eventType === 'UPDATE') {
        setFoodOrders(prev => prev.map(o => o.id === payload.new.id ? { ...o, ...payload.new, type: 'food' } : o));
      } else if (payload.eventType === 'INSERT') {
  ...
  const switchTab = (newTab) => {
    ...
    Animated.spring(tabAnim, { toValue: newTab === 'active' ? 0 : 1, useNativeDriver: false }).start();
    setTab(newTab);
  };
    ...
    if (type === 'dispute') {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setMktOrders(prev => prev.map(o => o.id === order.id ? { ...o, status: 'DISPUTED' } : o));
  ...
  const combined = [...mktOrders, ...foodOrders].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const activeOrders = combined.filter(o => !['COMPLETED', 'CANCELLED', 'DELIVERED'].includes(o.status));
  const historyOrders = combined.filter(o => ['COMPLETED', 'CANCELLED', 'DELIVERED'].includes(o.status));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      <View style={[styles.summaryRow, { backgroundColor: C.surface, borderBottomColor: C.edge }]}>
        ...
        <View style={styles.summaryItem}>
          <Text style={[styles.summaryNum, { color: C.primary }]}>
            ETB {fmtETB(historyOrders.reduce((s, o) => s + Number(o.total || 0), 0), 0)}
          </Text>
      ...
      <View style={[styles.tabContainer, { backgroundColor: C.surface, borderBottomColor: C.edge }]}>
            ...
            return (
              ...
              >
                <Text style={[styles.segmentText, { color: isActive ? C.text : C.sub }]}>
                  {tabKey === 'active' ? `ACTIVE (${activeOrders.length})` : `HISTORY (${historyOrders.length})`}
                </Text>
              </TouchableOpacity>
            );
          })}
        ...
        >
          {displayed.length === 0 ? (
            <View style={styles.emptyState}>
              <Ionicons name="receipt-outline" size={64} color={C.edge} style={{ marginBottom: 16 }} />
      ...
      <Modal visible={!!promptConfig} animationType="fade" transparent statusBarTranslucent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { backgroundColor: C.surface }]}>
            ...
            <Text style={{ fontFamily: Fonts.medium, fontSize: 14, color: C.sub, marginBottom: 20, lineHeight: 20 }}>
...
const styles = StyleSheet.create({
  ...
  tabIndicator: {
    position: 'absolute', width: '50%', top: 4, bottom: 4,
    borderRadius: 8, shadowColor: '#000', shadowOpacity: 0.08,
  ...
  countdownRow: {
    ...
    borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, marginBottom: 12,
  },
  countdownText: { fontFamily: Fonts.bold, fontSize: 12, color: '#f59e0b' },

  trackerContainer: { marginBottom: 16 },
  trackerSteps: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 10, paddingRight: 10 },
  trackerDot: { width: 14, height: 14, borderRadius: 7, borderWidth: 3 },
  trackerLine: { flex: 1, height: 2, marginHorizontal: -2 },
  trackerLabels: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 6, marginHorizontal: 5 },
  ...
  mapBtn: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },

  divider: { height: 1, marginVertical: 14 },

  escrowRow: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    paddingHorizontal: 10, paddingVertical: 8, borderRadius: 8, borderWidth: 1, marginBottom: 14,
  },
  ...
  modalInput: {
    borderRadius: 12, padding: 16, fontFamily: Fonts.medium, fontSize: 15,
  ...
  actionBtn: { paddingHorizontal: 20, paddingVertical: 12, borderRadius: 12, minWidth: 130, alignItems: 'center' },
});

  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      <View style={{ flexDirection: 'row', gap: 6, paddingHorizontal: 16, paddingVertical: 10 }}>
        {TONIGHT_CATS.map((c) => (
          <TouchableOpacity key={c} onPress={() => setTonightFilter(c)}
            style={{ paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20,
              backgroundColor: tonightFilter === c ? C.purpleL : C.lift,
              borderWidth: 1, borderColor: tonightFilter === c ? C.purple : C.edge2 }}>
            <Text style={{ fontSize: FontSize.xs, fontWeight: '600', color: tonightFilter === c ? C.purple : C.sub }}>
              {t(c, lang)}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <Modal visible={!!selected} transparent animationType="slide">
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} onPress={() => setSelected(null)}>
          <Pressable style={{ backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 }} onPress={e => e.stopPropagation()}>
            <Text style={{ fontSize: 48, textAlign: 'center', marginBottom: 12 }}>{selected?.icon || 'ðŸŽª'}</Text>
            ...
            {selected && [['Category', t(selected.cat, lang)],['Opens', selected.opens],['Closes', selected.closes],
              ['Entry', selected.entry > 0 ? `${selected.entry} ETB` : t('Free', lang)],['Rating', `â­ ${selected.rating || 4.5}`]
            ].map(([k,v]) => (
              <View key={k} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: C.edge }}>
                <Text style={{ color: C.sub }}>{k}</Text>
                <Text style={{ color: C.text, fontWeight: '700' }}>{v}</Text>
              </View>
            ))}
            <CButton title={selected?.entry > 0 ? t('Buy Ticket', lang) : t('Get Directions', lang)} onPress={() => setSelected(null)} style={{ marginTop: 20 }} />
            <CButton title={t('Cancel', lang)} onPress={() => setSelected(null)} variant="ghost" style={{ marginTop: 8 }} />
          </Pressable>
        </Pressable>
      </Modal>
    </View>

...
import {
  ...
  Platform, StatusBar, ActivityIndicator, Image, Animated as RNAnimated
} from 'react-native';
      ...
      const fetchLoc = async () => {
        ...
        if (data?.current_lat && data?.current_lng) {
          ...
          if (order.shipping_address_coords) {
             ...
             setEta(calculateETA(newLoc.latitude, newLoc.longitude, target.latitude, target.longitude));
          }
        }
      };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.overlay}>
        <BlurView intensity={Platform.OS === 'ios' ? 80 : 100} tint={isDark ? 'dark' : 'light'} style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.statusMain, { color: C.text }]}>{getStatusLabel(order?.status)}</Text>
              <Text style={[styles.statusSub, { color: C.sub }]}>Order #{order?.id?.slice(0, 8)}</Text>
            </View>
            {eta && (
          ...
          {order?.agent_id ? (
            <View style={styles.agentRow}>
               ...
               <View style={{ flex: 1 }}>
                  ...
                  <Text style={[styles.agentSub, { color: C.sub }]}>ID: {order.agent_id.slice(0, 6)}</Text>
               </View>
            ...
            <View style={styles.waitingRow}>
               ...
               <Text style={[styles.waitingText, { color: C.sub }]}>Allocating nearest delivery agent...</Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  card: {
    borderRadius: Radius['2xl'], padding: 20, overflow: 'hidden',
    ...Shadow.lg, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)',
  },
  ...
  progressTrack: { height: 4, backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: 2, marginTop: 20, marginBottom: 10 },
  ...
  // POD Styles
  podContainer: { marginTop: 15, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  podHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, padding: 8, backgroundColor: 'rgba(0,0,0,0.2)' },

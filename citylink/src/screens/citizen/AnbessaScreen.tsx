...
export default function AnbessaScreen() {
  ...
  async function handlePurchase() {
    if (!selTicket) return;
    if (!currentUser) {
      showToast('Sign in to purchase tickets', 'error');
      return;
    }
      ...
      >
        ...
        <View style={{ padding: 20 }}>
          <Card style={{ padding: 0 }}>
            <LinearGradient
              colors={isSheger ? [C.primary, C.primaryD] : [C.amber, '#E67E22']}
              style={{
                padding: 20,
            ...
            >
              ...
              <Text
                style={{ color: C.white, fontSize: 24, fontFamily: Fonts.black, marginTop: 12 }}
              >
                {liveBuses.length || 0} {t('buses_active')}
              </Text>
              <Text
                style={{ color: C.white, fontSize: 13, fontFamily: Fonts.medium, opacity: 0.8 }}
              >
                {t('tr_bus_status')}
              </Text>
            </LinearGradient>
                ...
                >
                  <View>
                    ...
                    <Text style={{ color: C.sub, fontSize: 11, fontFamily: Fonts.medium }}>
                      {t('approaching')} {b.stations[b.stIdx]}
                    </Text>
                  </View>
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={{ color: C.primary, fontSize: 14, fontFamily: Fonts.black }}>
                      {Math.ceil(b.etaSecs / 60)} min
                    </Text>
                    <Text style={{ color: C.sub, fontSize: 11, fontFamily: Fonts.medium }}>
                      {Math.round(b.occupancy * 100)}% {t('full')}
                    </Text>
                  </View>
                </View>
              ))}
            ...
            >
              <View
                style={{
                  ...
                  height: 48,
                  borderRadius: 12,
                  backgroundColor: selTicket?.id === t.id ? C.primaryL : C.lift,
                  alignItems: 'center',
              ...
              >
                <Ionicons
                  name={t.icon}
                  size={24}
                  color={selTicket?.id === t.id ? C.primary : C.sub}
                />
              </View>
              <View style={{ flex: 1 }}>
              ...
              <Text
                style={{
                  color: selTicket?.id === t.id ? C.primary : C.text,
                  fontSize: 18,
                  fontFamily: Fonts.black,
                }}
              >
                {t.price} <Text style={{ fontSize: 11 }}>ETB</Text>
              </Text>
            </TouchableOpacity>
          ))}
        </View>
          ...
          {ROUTES.map((r) => (
            <Card key={r.id} style={{ width: 220, padding: 16 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Text style={{ color: C.text, fontSize: 11, fontFamily: Fonts.bold }}>
                  {r.time}
                </Text>
              </View>
            </Card>

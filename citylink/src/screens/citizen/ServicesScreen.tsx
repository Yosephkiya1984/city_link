...
export function ServicesScreen() {
  ...
  async function handleBook() {
    if (!currentUser) { showToast(t('Sign in to book', lang), 'error'); return; }
    // Escrow payment required at booking.
    const depositAmount = 100;
    if (balance < depositAmount) { showToast(t('Insufficient balance for deposit', lang), 'error'); return; }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      <View style={{ flexDirection: 'row', gap: 8, padding: 16, paddingBottom: 8 }}>
        {[{v: 'salon', l: '💈 Salon'}, {v: 'clinic', l: 'ðŸ¥ Clinic'}].map(({v, l}) => (
          <TouchableOpacity key={v} onPress={() => setSvcType(v)}
            style={{ flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: Radius.lg,
              backgroundColor: svcType === v ? C.purpleL : C.lift, borderWidth: 1,
              borderColor: svcType === v ? C.purple : C.edge2 }}>
            <Text style={{ color: svcType === v ? C.purple : C.sub, fontWeight: '700' }}>{t(l, lang)}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 0, paddingBottom: 80 }}>
        {loading ? (
            ...
            <EmptyState icon={svcType === 'salon' ? 'ðŸ’‡â€â™€ï¸' : 'ðŸ©º'} title={`No ${svcType}s found`} subtitle="Merchants are registering now." />
        ) : providers.map((p) => (
          <View key={p.id} style={{ backgroundColor: C.surface, borderRadius: Radius.xl,
            borderWidth: 1, borderColor: C.edge, padding: 16, marginBottom: 10 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <View>
                <Text style={{ color: C.text, fontSize: FontSize.xl, fontWeight: '700' }}>{p.merchant_name}</Text>
                <Text style={{ color: C.sub, marginTop: 2 }}>ðŸ“ Bole Â· â­ 4.7</Text>
              </View>
            </View>
            <CButton title={t('Book Appointment', lang)} onPress={() => setBookingModal(p)}
      ...
      <Modal visible={!!bookingModal} transparent animationType="slide">
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} onPress={() => setBookingModal(null)}>
          <Pressable style={{ backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 }} onPress={e => e.stopPropagation()}>
            <Text style={{ color: C.text, fontSize: FontSize['2xl'], fontWeight: '800', marginBottom: 8 }}>
            ...
            <CButton title={t('Pay 100 ETB & Reserve', lang)} onPress={handleBook} style={{ marginTop: 8 }} />
            <CButton title={t('Cancel', lang)} onPress={() => setBookingModal(null)} variant="ghost" style={{ marginTop: 8 }} />
          </Pressable>
        </Pressable>
      </Modal>
    </View>

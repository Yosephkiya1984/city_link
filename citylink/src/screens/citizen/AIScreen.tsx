...
const AI_INSIGHTS = [
  {
    id: 'usage',
    title: 'Usage Pattern',
    insight: 'You use transport features most often in the morning',
    recommendation: 'Consider setting up quick access to LRT schedules',
    icon: 'analytics',
    color: '#6366f1',
  },
  {
    ...
    insight: 'You could save 15% on transport with monthly passes',
    recommendation: 'Check out our transport subscription plans',
    icon: 'trending-up',
    color: '#10b981',
  },
  {
...
export default function AIScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      <TopBar title="ðŸ¤– CityLink AI" right={
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity onPress={() => setShowInsights(!showInsights)}
            style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: Radius.lg,
              backgroundColor: showInsights ? C.primaryL : C.lift, borderWidth: 1, borderColor: C.edge2 }}>
            <Text style={{ color: showInsights ? C.primary : C.sub, fontSize: FontSize.xs, fontWeight: '700' }}>Insights</Text>
          </TouchableOpacity>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}
              ...
              {AI_INSIGHTS.map((insight) => (
                <TouchableOpacity key={insight.id} style={{
                  width: 200, backgroundColor: C.lift, borderRadius: Radius.xl,
                  padding: 12, borderWidth: 1, borderColor: C.edge2, ...Shadow.md
                }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: insight.color + '20', alignItems: 'center', justifyContent: 'center' }}>
                      <Ionicons name={insight.icon} size={16} color={insight.color} />
                    </View>
                    <Text style={{ color: C.text, fontSize: FontSize.sm, fontFamily: Fonts.black }}>{insight.title}</Text>
                  </View>
                  <Text style={{ color: C.sub, fontSize: FontSize.xs, fontFamily: Fonts.medium, marginBottom: 4 }}>{insight.insight}</Text>
                  <Text style={{ color: C.primary, fontSize: FontSize.xs, fontFamily: Fonts.bold }}>{insight.recommendation}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
          ...
          {CATEGORIES.map((cat) => (
            <TouchableOpacity key={cat.id} onPress={() => setSelectedCategory(cat.id)}
              style={{
                ...
                borderWidth: 1, borderColor: selectedCategory === cat.id ? C.primary : C.edge2,
                flexDirection: 'row', alignItems: 'center', gap: 6,
              }}>
              <Ionicons name={cat.icon} size={14} color={selectedCategory === cat.id ? C.white : C.sub} />
              <Text style={{ color: selectedCategory === cat.id ? C.white : C.sub, fontSize: FontSize.sm, fontWeight: '600' }}>
                {cat.name}
              </Text>
            </TouchableOpacity>
          ))}
          ...
          {chatHistory.map((msg) => (
            <View key={msg.id}
              ...
              <View style={{ maxWidth: '78%' }}>
                ...
                <Text style={{ color: C.sub, fontSize: FontSize.xs, marginTop: 3,
                  paddingHorizontal: 4, textAlign: msg.role === 'user' ? 'right' : 'left' }}>
                  {fmtTime(msg.ts)}
                </Text>
              </View>
            </View>
        ...
        {chatHistory.length <= 1 && (
          <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
            <Text style={{ color: C.sub, fontSize: FontSize.sm, fontFamily: Fonts.medium, marginBottom: 8 }}>
              Quick Actions
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {filteredSuggestions.map((suggestion) => (
                <TouchableOpacity key={suggestion.id} onPress={() => handleSend(suggestion.text)}
                  style={{
                    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
                    backgroundColor: C.lift, borderWidth: 1, borderColor: C.edge2,
                    flexDirection: 'row', alignItems: 'center', gap: 6,
                  }}>
                  <Text style={{ fontSize: 16 }}>{suggestion.icon}</Text>
                  <Text style={{ color: C.sub, fontSize: FontSize.sm, fontWeight: '600' }}>{suggestion.text}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
        ...
        <View style={{
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              borderWidth: 1, borderColor: voiceMode ? C.primary : C.edge2,
            }}>
          ...
          <TouchableOpacity
            ...
            <Text style={{ fontSize: 18, color: (!input.trim() || loading) ? C.sub : '#040A05' }}>â†‘</Text>
          </TouchableOpacity>
        </View>

...
export function MinibusScreen() {
  ...
  const filteredRoutes = routes.filter((r) => {
    const ms =
      searchFrom.trim() === '' ||
      (r.from_city && r.from_city.toLowerCase().includes(searchFrom.toLowerCase()));
    const me =
      searchTo.trim() === '' ||
      (r.to_city && r.to_city.toLowerCase().includes(searchTo.toLowerCase()));
    return ms && me;
  });
    ...
    <View style={{ flex: 1, backgroundColor: C.ink }}>
      ...
      >
        ...
        >
          ...
          <TextInput
            placeholder={t('Origin (e.g. Bole)', lang)}
          ...
          />
          <TextInput
            placeholder={t('Destination (e.g. Piassa)', lang)}
            placeholderTextColor={C.sub}
            value={searchTo}
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            subtitle="Try searching without specific origin/destination."
          />
        ) : (
          filteredRoutes.map((route) => (
            <TouchableOpacity
              key={route.id}
              onPress={() => setSelected(selected?.id === route.id ? null : route)}
              style={{
                marginHorizontal: 16,
                marginBottom: 10,
                backgroundColor: C.surface,

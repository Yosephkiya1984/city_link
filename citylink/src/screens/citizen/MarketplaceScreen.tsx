...
import {
  ...
  Animated, Image, TextInput, RefreshControl, StyleSheet,
  ...
  return (
    <View style={styles.header}>
      <View>
        ...
        <Text style={styles.headerTitle}>CityLink <Text style={{ color: T.primary }}>Market</Text></Text>
      </View>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        <TouchableOpacity style={styles.headerBtn} onPress={() => (navigation as any).navigate('MyOrders')}>
    ...
    {CATEGORIES.map(cat => {
      ...
      return (
        <TouchableOpacity
          ...
          style={[styles.pill, active && { backgroundColor: cat.color + '22', borderColor: cat.color + '60' }]}
        >
          <Ionicons name={cat.icon as any} size={13} color={active ? cat.color : T.textSub} />
          <Text style={[styles.pillText, active && { color: cat.color, fontWeight: '700' }]}>{cat.name}</Text>
        </TouchableOpacity>
      );
    })}
  </ScrollView>
    ...
    >
      <Animated.View style={[styles.featuredCard, { transform: [{ scale }] }]}>
        {img ? (
          ...
          <View style={[styles.featuredImg, { backgroundColor: T.surfaceHigh, alignItems: 'center', justifyContent: 'center' }]}>
        ...
        <View style={styles.featuredOverlay}>
          ...
          <Text style={styles.featuredName} numberOfLines={2}>{item.name || item.title}</Text>
      ...
      <Animated.View style={[styles.productCard, { transform: [{ scale }] }]}>
        <View style={styles.productImgWrap}>
          {img ? (
            ...
            <View style={[styles.productImg, { backgroundColor: T.surfaceHigh, alignItems: 'center', justifyContent: 'center' }]}>
              <Ionicons name="cube-outline" size={32} color={T.textMuted} />
            </View>
          )}
          {soldOut && (
            <View style={[styles.stockLabel, { backgroundColor: T.redDim, borderColor: T.red + '40' }]}>
              <Text style={[styles.stockLabelText, { color: T.red }]}>SOLD OUT</Text>
            </View>
          )}
          {lowStock && (
            <View style={[styles.stockLabel, { backgroundColor: T.secondaryDim, borderColor: T.secondary + '40' }]}>
        ...
        <View style={styles.productBody}>
          ...
          <View style={styles.productFooter}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { if (!soldOut) { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); onPress(item); } }}
              disabled={soldOut}
            >
...
export default function MarketplaceScreen({ navigation: nav }) {
  ...
  const handleBuy = async () => {
    ...
    if (!user) { showToast('Please login to purchase', 'error'); return; }
    if (user.id === selectedProduct?.merchant_id) { showToast("You can't buy your own listing", 'warning'); return; }
    const currentBalance = useAppStore.getState().balance || 0;
    const totalCost = (selectedProduct?.price || 0) * qty;
    if (currentBalance < totalCost) { showToast(`Insufficient balance (ETB ${fmtETB(totalCost, 0)} needed)`, 'error'); return; }
    if ((selectedProduct?.stock || 0) < qty) { showToast('Not enough stock', 'error'); return; }
    ...
    try {
      const res = await (marketplaceService as any).completePurchase({
        ...
        buyerId: user.id,
        qty,
        shippingAddress: `${(user as any).subcity || 'Addis Ababa'}, ${(user as any).woreda || 'City Center'} - ${deliveryInstructions}`.trim(),
      });
      if (res.success) {
  ...
  const renderProductItem = useCallback(({ item }) => (
    <ProductCard
      item={item}
      onPress={p => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setQty(1); setSelectedProduct(p); }}
    />
  ), []);

  const renderSkeleton = () => (
    <View style={styles.grid}>
      {[1, 2, 3, 4].map(k => <ProductSkeleton key={k} />)}
    </View>
  );

  const sellerName = selectedProduct
    ? (selectedProduct.business_name || selectedProduct.merchant_name || selectedProduct.seller_name || 'Verified Seller')
    : '';
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={{ paddingLeft: 20, marginBottom: 24 }}>
          <CategoryPills selected={category} onSelect={cat => { setCategory(cat); setLoading(true); }} />
        </View>
        ...
        {!loading && search === '' && category === 'All' && featured.length > 0 && (
          <View style={{ marginBottom: 32 }}>
            ...
            >
              {featured.map(item => (
                <FeaturedCard key={item.id} item={item} onPress={p => { setQty(1); setSelectedProduct(p); }} />
              ))}
            </ScrollView>
          </View>
        )}
      ...
      <Modal visible={!!selectedProduct} animationType="slide" transparent statusBarTranslucent>
        <View style={styles.modalBg}>
          <TouchableOpacity style={{ flex: 1 }} onPress={() => setSelectedProduct(null)} activeOpacity={1} />
                ...
                <>
                  ...
                  <View style={styles.modalImgWrap}>
                    {img ? (
                      ...
                      <View style={[styles.modalImg, { backgroundColor: T.surfaceHigh, alignItems: 'center', justifyContent: 'center' }]}>
                    ...
                    {selectedProduct.condition && (
                      <View style={styles.conditionBadge}>
                        <Text style={styles.conditionText}>{selectedProduct.condition.toUpperCase()}</Text>
                      </View>
                    )}
                  </View>

                  <ScrollView style={{ maxHeight: SH * 0.55 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 24, paddingBottom: 40 }}>
                    {/* Category + Name */}
                    <Text style={styles.modalCat}>{(selectedProduct.category || 'GENERAL').toUpperCase()}</Text>
                    <Text style={styles.modalTitle}>{selectedProduct.name || selectedProduct.title}</Text>
                    ...
                    <View style={styles.buyRow}>
                      <View>
                        ...
                        <Text style={styles.priceValue}>ETB {fmtETB(selectedProduct.price, 0)}</Text>
                        {qty > 1 && (
                      ...
                      <View style={styles.qtyPicker}>
                        <TouchableOpacity
                          style={styles.qtyBtn}
                          onPress={() => { if (qty > 1) { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setQty(qty - 1); } }}
                          disabled={qty <= 1}
                        >
                          <Ionicons name="remove" size={18} color={qty <= 1 ? T.textMuted : T.text} />
                        </TouchableOpacity>
                        <Text style={styles.qtyText}>{qty}</Text>
                        <TouchableOpacity
                          style={styles.qtyBtn}
                          onPress={() => { if (qty < Math.min(selectedProduct.stock, 10)) { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setQty(qty + 1); } }}
                          disabled={qty >= Math.min(selectedProduct.stock, 10)}
                        >
                          <Ionicons name="add" size={18} color={qty >= Math.min(selectedProduct.stock, 10) ? T.textMuted : T.text} />
                        </TouchableOpacity>
                      </View>
                    </View>
                    ...
                    <Text style={[styles.stockInfo, { color: soldOut ? T.red : (selectedProduct.stock <= 5 ? T.secondary : T.textSub) }]}>
                      ...
                      <Text style={{ color: T.red, fontSize: 12, marginBottom: 12, marginTop: -4 }}>
                        Insufficient balance â€” need ETB {fmtETB(totalCost - (balance as any || 0), 0)} more
                      </Text>

                    <TouchableOpacity
                      style={[styles.buyNowBtn, (buying || soldOut || !canAfford) && { opacity: 0.5 }]}
                      onPress={handleBuy}
                    ...
                    >
                      {buying ? (
                        ...
                        <>
                          <Ionicons name="lock-closed" size={16} color={T.bg} style={{ marginRight: 8 }} />
                          <Text style={styles.buyNowText}>
                            {soldOut ? 'Out of Stock' : !canAfford ? 'Insufficient Balance' : `Buy Now Â· ETB ${fmtETB(totalCost, 0)}`}
                          </Text>
                        </>
                      )}
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  headerGreeting: { fontSize: 11, color: T.textSub, fontWeight: '600', letterSpacing: 0.3, marginBottom: 2 },
  headerTitle: { fontSize: 22, fontWeight: '800', color: T.text, letterSpacing: -0.5 },
  headerBtn: {
    width: 38, height: 38, borderRadius: 10, backgroundColor: T.surface,
    borderWidth: 1, borderColor: T.border, alignItems: 'center', justifyContent: 'center',
  },
  balancePill: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    backgroundColor: T.primaryDim, borderWidth: 1, borderColor: T.primary + '40',
  ...
  featuredBadge: { backgroundColor: T.primary + '20', borderWidth: 1, borderColor: T.primary + '60', alignSelf: 'flex-start', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3, marginBottom: 8 },
  ...
  lowStockBadge: { position: 'absolute', top: 12, right: 12, backgroundColor: T.secondaryDim, borderWidth: 1, borderColor: T.secondary + '60', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
  lowStockText: { fontSize: 9, fontWeight: '800', color: T.secondary },

  // Product grid
  grid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 12 },
  productCard: {
    width: (SW - 52) / 2, backgroundColor: T.card,
  ...
  productCat: { fontSize: 9, color: T.textSub, fontWeight: '700', letterSpacing: 0.8, marginBottom: 4 },
  ...
  modalSheet: {
    ...
    overflow: 'hidden', maxHeight: SH * 0.92,
  },
  ...
  modalCat: { fontSize: 10, color: T.primary, fontWeight: '700', letterSpacing: 1, marginBottom: 6 },
  modalTitle: { fontSize: 24, fontWeight: '800', color: T.text, letterSpacing: -0.5, marginBottom: 10 },
  ...
  sellerLabel: { fontSize: 9, color: T.textSub, fontWeight: '700', letterSpacing: 0.8, marginBottom: 2 },
  ...
  escrowBadge: {
    ...
    backgroundColor: T.primaryDim, borderWidth: 1, borderColor: T.primary + '30',
  ...
  buyRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8 },
  priceLabel: { fontSize: 10, color: T.textSub, fontWeight: '700', letterSpacing: 1, marginBottom: 4 },
  ...
  buyNowBtn: {
    backgroundColor: T.primary, borderRadius: 14, paddingVertical: 16,
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
  },
  buyNowText: { color: T.bg, fontSize: 15, fontWeight: '800' },
  deliveryInputBox: { marginTop: 16, marginBottom: 4 },
  deliveryInput: { backgroundColor: T.bg, borderRadius: 8, padding: 12, color: T.text, marginTop: 8, borderWidth: 1, borderColor: T.border, fontSize: 13 }
});
